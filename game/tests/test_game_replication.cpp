...
public:
    snt::core::Expected<GameAuthenticatedPeer> authenticate(
        ...
        return GameAuthenticatedPeer{.player_id = player_id};
    }

    void on_peer_disconnected(const GameAuthenticatedPeer& peer,
                              std::string_view reason) noexcept override {
        disconnected_player_id = peer.player_id;
        disconnect_reason = reason;
    }

    std::string player_id = "player:alice";
    bool reject = false;
...
TEST(GameReplicationHandlerTest, AuthenticatesThenQueuesCommandsAndAcknowledgesLogin) {
    ...
    EXPECT_EQ(accepted->player_id, authenticator.player_id);
    ...
    EXPECT_EQ(command_sink.last_player_id, authenticator.player_id);
    ...
    EXPECT_EQ(authenticator.disconnected_player_id, authenticator.player_id);
