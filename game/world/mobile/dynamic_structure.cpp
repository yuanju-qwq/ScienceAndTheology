...
int floor_div_chunk(int value) {
    return static_cast<int>(
        std::floor(static_cast<float>(value) / VoxelChunk::kChunkSize));
}
...
ResolvedCell resolve_cell(
    ...
    out.local_x = block_x - out.chunk_x * VoxelChunk::kChunkSize;
    out.local_y = block_y - out.chunk_y * VoxelChunk::kChunkSize;
    out.local_z = block_z - out.chunk_z * VoxelChunk::kChunkSize;
...
const TerrainCell* resolve_const_cell(
    ...
    const int lx = block_x - cx * VoxelChunk::kChunkSize;
    const int ly = block_y - cy * VoxelChunk::kChunkSize;
    const int lz = block_z - cz * VoxelChunk::kChunkSize;

    const VoxelChunk* chunk = world.get_chunk(dimension_id, cx, cy, cz);
...
} // namespace snt::game::mobile_structure
