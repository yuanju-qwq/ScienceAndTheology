...
| `game/client/main.cpp` | 应用程序入口，配置显式运行时路径并创建 `ClientRuntime + ScienceAndTheologySession`。 |
...
`snt_tests` 覆盖通用引擎模块并包含 P6 保留式 UI 测试；`snt_simulation_runtime_tests` 只链接 SDL/Vulkan-free 模拟闭包。`snt_p6_tests` 不再存在。
