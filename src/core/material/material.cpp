...
bool Material::generates_form(MaterialForm form) const {
    ...
    // Plates may also be generated for non-metal industrial materials
    // such as glass.
    if (f == static_cast<uint16_t>(MaterialForm::PLATE) ||
        f == static_cast<uint16_t>(MaterialForm::DOUBLE_PLATE) ||
        f == static_cast<uint16_t>(MaterialForm::DENSE_PLATE)) {
        return (generation_flags & MaterialGenFlag::METAL) != 0 ||
               (generation_flags & MaterialGenFlag::PLATE) != 0;
    }

    // Rods may be generated by non-metal structural materials such as wood.
    if (f == static_cast<uint16_t>(MaterialForm::ROD) ||
        f == static_cast<uint16_t>(MaterialForm::LONG_ROD)) {
        return (generation_flags & MaterialGenFlag::METAL) != 0 ||
               (generation_flags & MaterialGenFlag::ROD) != 0;
    }

    // Metal fasteners and mechanical parts.
    if (f == static_cast<uint16_t>(MaterialForm::BOLT) ||
    ...
    uint8_t elem_count, const ElementComposition* composition)
{
    ...
    const char* stable_name = intern_string(name);
    const char* stable_title_key = intern_string(title_key);
    const char* stable_chemical_formula = intern_string(chemical_formula);

    // 幂等检查：若 name 已存在，直接返回，保持旧映射不变。
    // - 传入 id 与已有 id 相同：幂等返回，避免覆盖开销。
    // - 传入 id 与已有 id 不同：保持旧映射，避免破坏既有索引。
    if (stable_name[0] != '\0') {
        auto it = g_material_name_to_id.find(stable_name);
    ...
    entry.material.name = stable_name;
    entry.material.title_key = stable_title_key;
    ...
    entry.material.chemical_formula = stable_chemical_formula;
