...
GlyphId GlyphRegistry::register_glyph(const GlyphDef& def, GlyphId explicit_id) {
    ...
    g_glyph_registry[id] = def;
    g_glyph_name_map[def.name] = id;

    // Update index based on slot_type.
    if (def.slot_type == GlyphSlotType::FORM) {
        g_form_glyph_index[static_cast<int>(def.form)] = id;
    } else if (def.slot_type == GlyphSlotType::EFFECT) {
        g_effect_glyph_index[static_cast<int>(def.element)][static_cast<int>(def.tier)] = id;
    } else if (def.slot_type == GlyphSlotType::AUGMENT) {
        g_augment_glyph_index[static_cast<int>(def.element)][static_cast<int>(def.tier)] = id;
    }
    return id;
}
