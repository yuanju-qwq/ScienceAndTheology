...
//
// Single-player mode: exactly one player with id = kSinglePlayerId (1).
//
...
public:
    ...
    // bind_equipment.
    bool register_player(PlayerId id,
    ...
    bool unregister_player(PlayerId id);

    // Bind / rebind the inventory pointer for a registered player.
    bool bind_inventory(PlayerId id, gt::Inventory* inventory);

    // Bind / rebind the equipment pointer for a registered player.
    bool bind_equipment(PlayerId id, gt::Equipment* equipment);

    // Update the player's current chunk position.
    bool set_player_chunk(PlayerId id,
