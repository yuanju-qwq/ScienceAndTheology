...
func _register_materials() -> void:
	# Each entry: [id_suffix, display_name, color_hex,
	# state_str, melt_K, boil_K, mass, formula, subcategory]
	var mat_data: Array = [
		# ── Primitive / Stone-age ──
		...
		["stainless_steel", "Stainless Steel",
			0xC8C8DC, _STATE_SOLID, 1700, 0, 55.4, "Fe9CrNiMn", "alloys"],
		...
		["annealed_copper", "Annealed Copper", 
		 	0xFF8C42, _STATE_SOLID, 1000, 0, 63.5, "Cu", "alloys"],
		...
		["naquadah_alloy", "Naquadah Alloy", 
			0x006600, _STATE_SOLID, 7200, 0, 310.0, "NqAl", "alloys"],
		# ── Gems ──
		...
		["sulfuric_acid", "Sulfuric Acid", 
			0xCCAA00, _STATE_LIQUID, 283, 610, 98.0, "H2SO4", "liquids"],
		["hydrochloric_acid", "Hydrochloric Acid",
			0x99CC99, _STATE_LIQUID, 188, 383, 36.5, "HCl", "liquids"],
		["nitric_acid", "Nitric Acid", 0xCCCC00, _STATE_LIQUID, 231, 356, 63.0, "HNO3", "liquids"],
		["hydrofluoric_acid", "Hydrofluoric Acid",
			0x88CC88, _STATE_LIQUID, 190, 393, 20.0, "HF", "liquids"],
		...
		["nitrogen_dioxide", "Nitrogen Dioxide", 
			0xCC6600, _STATE_GAS, 262, 294, 46.0, "NO2", "gases"],
		...
		["hydrogen_sulfide", "Hydrogen Sulfide", 
			0xCCCC00, _STATE_GAS, 187, 213, 34.0, "H2S", "gases"],
		...
		["acetic_acid", "Acetic Acid",
			0xCCBB66, _STATE_LIQUID, 290, 391, 60.0, "C2H4O2", "organic"],
		...
		["vinyl_chloride", "Vinyl Chloride",
			0xCCBBCC, _STATE_GAS, 119, 260, 62.5, "C2H3Cl", "organic"],
...
func _register_items() -> void:
	...
	for t: Array in gt_tools:
		...
		if item_def:
			...
			var icon: Texture2D = item_def.icon if item_def.icon \
				else make_placeholder_icon(Color.hex(int(str(t[2]))))
			var e := WikiEntry.new()
			e.id = eid
			e.category = "items"
			e.icon = icon
...
func _register_magic() -> void:
	...
	for element: String in rune_colors:
		...
		_reg("magic.runes_" + ename, "magic", Color.hex(col),
			...
			[{"label": "Element", "value": cap_name},
				{"label": "Tiers", "value": "Common, Refined, Superior, Legendary"}])
...
func _register_guides() -> void:
	...
	_reg("guide.crafting", "guides", Color(0xCC8844),
		...
		[{"label": "Stations", "value": "Hand, Workbench, Furnace"},
		 {"label": "Categories",
			"value": "Materials, Tools, Parts, Wires, Cables, Circuits, Machines"}])
