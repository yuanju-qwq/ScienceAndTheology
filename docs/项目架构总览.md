...
| CMake 直接依赖审计 | 已实现；配置期检查 21 个引擎模块的 134 条内部 include 边 |
| asset catalog / GPU residency | `FilesystemAssetSource` 与 source-backed `AssetCatalog` 已实现并覆盖根目录、路径逃逸、manifest 和稳定 ID 测试；`IGpuAssetUploader` 仍为接口声明，当前 `AssetManager` 仍负责 Vulkan mesh 缓存 |
...
- Asset source：`FilesystemAssetSource` 是 `IAssetSource` 的首个实现，拥有显式内容根并拒绝任何带根的路径和根目录逃逸；可在非渲染加载线程使用，不访问 `World` 或全局服务，尚未注入 Runtime。
- Asset catalog：`AssetCatalog` 通过 source 读取 manifest，将稳定内容 ID 解析为相对 source request；解析和查询后不访问文件/GPU，尚未注入 Runtime。
- GPU residency：`IGpuAssetUploader` 在 render thread/device 生命周期内按值接收上传请求，返回 Vulkan-free token 并显式提供 release/evict；目前仍只声明，`AssetManager` 尚未迁移。
