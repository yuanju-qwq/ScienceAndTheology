...
`game/runtime/runtime_package.*` 统一定位可执行程序、构造三个显式根目录，并从同一个游戏包配置读取 `RuntimeConfig` 与 `GameSessionConfig`。`game/client/main.cpp` 先通过 `snt_game_identity` 解析 Steam 或本地玩家账号，再创建 `ClientRuntime + ScienceAndTheologyClientSession`；`game/server/main.cpp` 创建 `SimulationRuntime + ScienceAndTheologyServerSession`。二者复用 `snt_game_simulation` 中的脚本、机器 worker 和 terrain/sidecar 初始化：
...
└── user/                      # 运行时可写
    ├── logs/
    ├── profile/                # local_player_identity.bin (`SNTI` v1)
    ├── saves/
    └── cache/
```
...
| 任务运行态 | P7.3 core 已实现；P7.4 任务进度 provider 已实现：per-player `SNTQ` v1 文件、严格读取和 backup recovery。真实玩家/机器事件 producer、reward effect、玩家生命周期调用点和机器运行态持久化未完成 |
| 其余工业/魔法迁移 | 未完成，旧代码仍是迁移参考 |
| 玩家账号身份 | 已实现 `snt_game_identity`：Steam 为 `steam:<SteamID64>`，本地为 `local-name:<validated UTF-8 name>`；本地首启输入并保存 `SNTI` v1 profile，同显示名的 Steam/local 用户互不共享账号或任务进度；Steamworks adapter 待接入 |
...
| replication transport 和服务端权威联机 | `snt_network` 已实现 TCP reliable + UDP unreliable、wire codec、主线程 tick 编排和 Steam P2P adapter；`snt_game_network` 已实现 `SNTG` v2、local-name 准入和同账号后登录接管；Steam ticket verifier、client connection、gameplay command、AOI、snapshot/delta 仍待实现 |
| 音频 | 未实现 |
...
- Quest progress：`QuestRegistry` 以稳定 game account id + quest/objective id 保存值记录；`QuestPlayerId` 直接使用 `steam:<SteamID64>` 或 `local-name:<validated UTF-8 name>`，因此同显示名的 Steam/local 用户不共享进度，本地同名用户则有意共享。`quest_progress.h` 是不依赖 simulation 的值/接口边界。`GameSaveQuestProgressPersistence` 经 `GameSaveManager` 在显式 universe save 根的 `players/` 写入 `SNTQ` v1 per-player 文件；调用仍由未来 player lifecycle 触发，fixed tick 不做文件 I/O。
- 用户数据：只能写入 `user_root`。
...
- [任务系统设计.md](任务系统设计.md)：任务定义、运行态、热重载和 P7.4 存档边界。
...
- 核对范围：包含 `SimulationRuntime + ISimulationSession`、`ClientRuntime + IClientSession`、SimulationRuntime scheduler、P7.2.1 gameplay、P7.3 QuestRegistry core、P7.4 task progress provider、game account identity、P0 CMake 依赖隔离，以及 A 方案 voxel/game world 数据边界
- 更新日期：2026-07-14
