...
flowchart TD
    ...
    Host --> GameWorld["game/world + worldgen + save + mobile"]
    Host --> Content["game/config + scenes + scripts + assets"]

    Runtime --> Presentation["render + renderer + render_backend + ui"]
    Runtime --> Simulation["ecs + voxel_data/storage + voxel + player + script"]
...
| CMake 直接依赖审计 | 已实现；配置期检查 18 个引擎模块的 131 条内部 include 边 |
| asset catalog / GPU residency | Runtime 已拥有 `FilesystemAssetSource` 与 source-backed `AssetCatalog`，并注入 AssetManager/RuntimeServices；`VulkanGpuAssetUploader` 已实现 `IGpuAssetUploader`，以 token lease 管理 mesh GPU 常驻，legacy mesh cache/loader 与无生产调用的 `AssetCache` 已删除 |
...
- GPU residency：`VulkanGpuAssetUploader` 在 render thread/device 生命周期内实现 `IGpuAssetUploader`；它按 canonical source 共享底层 mesh、为每个调用者返回独立 token lease，并由 `AssetManager` 在 device wait-idle 后执行 release/evict。场景仅通过 `IMeshAssetReferenceResolver` 使用稳定 `MeshHandle`，不接触 Vulkan。
- Generic voxel data：`snt_voxel_data` 只拥有 terrain、`VoxelChunk`、`ChunkKey` 和 `ChunkRegistry`；Runtime、renderer、player 只处理这一层。
...
- 固定引擎子模块基线：`82a329f10dbd525b2651e50e961e6f8e029d807f`
- 核对范围：包含 `Runtime + IGameSession`、Runtime scheduler、P7.2.1 gameplay、P0 CMake 依赖隔离，以及 A 方案 voxel/game world 数据边界
- 更新日期：2026-07-14
