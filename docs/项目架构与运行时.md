...
> 当前架构基线，更新于 2026-07-24。
>
> 当前工作区包含 `SimulationRuntime + ISimulationSession`、`ClientRuntime + IClientSession`、`snt_game_server`、`snt_network`、`snt_game_identity`、`snt_game_network`、`snt_game_player_motion`、game-owned P7.2.4 多输入/手动激活机器运行态、activation context 边界与原始工业内容、P7.3 `QuestRegistry`、Better Questing 式前置自动解锁、结构化 reward 与 committed event/reward bridge、P7.4 task progress provider、dedicated-server 玩家生命周期、共享世界存档生命周期与 chunk-sidecar 机器恢复/保存、P7.5 `SNTG` v20 snapshot/delta/typed command/continuous player-motion codec、可选服务器口令、原生 IPv4 LAN discovery、budgeted provider scheduling、player/loaded-chunk/machine concrete AOI/source、server-owned movement、独立 UDP movement sequence、player v2 连续复制、本地 prediction/ACK 回放/帧级平滑、远端插值缓存、RLE terrain payload、只读任务书与机器缓存、图形客户端 remesh handoff，以及主机权威/客户端可信提示的方块、机器和任务奖励领取、sidecar 床/墓碑/重生服务、P0 CMake 依赖隔离和 A 方案 voxel/game world 数据边界变更。
...
| P1 | 游戏账号身份、`SNTG` v20、登录准入、客户端 local-name 连接闭环、前置自动解锁与显式 QuestClaimReward、已提交任务 event/reward bridge、主机权威/客户端可信提示的 BlockInteraction、认证图形客户端 DDA 交互绑定、独立 UDP movement intent、任务/玩家存档生命周期、player/loaded-chunk/machine AOI snapshot/delta、player v2 连续复制、本地 prediction/ACK 回放/帧级平滑、远端插值与原生 avatar、认证账号任务书只读值同步、MUI 任务书、生产机器输入槽编辑和捕食者战斗死亡 producer 已实现；Steamworks 平台 backend、墓碑 UI 尚未实现，音频尚不存在 | `snt_game_identity` 以 `steam:<SteamID64>` / `local-name:<name>` 隔离账号；`snt_game_network` 已实现 typed login/command/movement/snapshot/delta codec、会话接管、budgeted `GameServerReplicationHandler` 与可注入 transport 的 `GameClientReplicationSession` value drain；`snt_game_player_motion` 让 `GameServerPlayerMovement` 与客户端预测共用碰撞、跳跃和重力规则，`GameServerPlayerReplication` 从 server ECS、连续 motion source、loaded `ChunkRegistry`、sidecar anchor/live machine runtime 和 typed value source 生成四类 AOI/delta baseline；`GameRemotePlayerWorld`、`GameClientRemoteChunkWorld`、`GameRemoteMachineWorld` 与 `GameClientQuestBookState` 应用 presentation-only 值，`GameClientPlayerPrediction` 与 `GameRemotePlayerInterpolator` 分别把 player v2 值交给相机和 `GameRemotePlayerPresentationWorld`，后者维护 transient full/LOD avatar ECS 实体；server composition 还拥有 sidecar bed/grave/respawn/death、online player combat、block/machine interaction 与 quest event/reward services，捕食者值伤害已接入死亡事务 |
...
| `snt_network` | transport-neutral replication、wire codec、non-blocking TCP+UDP、IPv4 LAN discovery、Steam P2P adapter | 只依赖 `snt_core`（Windows 链接 `ws2_32`）；无 SDL/Vulkan/Steamworks SDK；Steam SDK 集成实现 `ISteamP2PBackend` 后注入 |
| `snt_game_network` | 游戏 `SNTG` v20 envelope、独立可选服务器口令、typed login/command/unreliable movement/snapshot/delta codec、兼容协议 LAN browser、client/server 准入 handler、认证/命令/AOI/source contracts、typed account value、player v2、RLE terrain 与 machine v1 presentation codec | game-owned 且 headless；`GameAccountPeerAuthenticator` 已允许 local-name 并让同 account 新登录接管旧 peer，且可在身份映射前验证 runtime-injected server password；handler 对注入 provider 执行严格 codec + bytes/chunk/entity/value/block per-peer budget + atomic queue，`GameClientReplicationSession` 为 reliable command 和 UDP movement 分别维护递增 sequence，`GameLanDiscoveryClient` 只输出当前协议服务器，图形客户端 `LanServerBrowserModel` 与直接 MUI 服务器列表在选择后才建立 transport，`GameRemotePlayerWorld`、`GameClientRemoteChunkWorld`、`GameRemoteMachineWorld` 与 `GameClientQuestBookState` 只应用有序 presentation value；Steam ticket verifier 尚未接入 |
| `snt_game_player_motion` + `game/client/player_motion_presentation.*` + `game/client/remote_player_presentation_world.*` | 服务端与客户端共用的连续玩家移动内核、本地 prediction/reconciliation、远端插值值缓存与原生 avatar 投影 | shared motion target 只依赖 typed movement、窄 voxel collision view 和 game replication value；`GameServerPlayerMovement` 是唯一权威写入者，客户端只预测已提交输入并以 player v2 ACK 回放；插值模块不持有 transport、renderer、ECS World 或持久状态，`GameRemotePlayerPresentationWorld` 是唯一消费其采样值并维护 transient renderer 实体的客户端层 |
...
| `game/server/game_server_command_sink.*` | `QuestClaimReward` 与 `BlockInteraction` 的 reliable command sequence、独立 UDP movement sequence、稳定排序、latest movement intent 合并和断线取消 | dedicated-server composition-owned；任务状态只由 `QuestRegistry` 前置图和已提交玩法事件自动变更；命令 sink 只借用 `QuestRegistry`、narrow movement input sink 与 interaction service，不持有 transport/ECS World；未知命令是协议拒绝，任务规则拒绝以低频聚合 Warn 记录 |
...
| `game/server/game_server_player_movement.*` | authenticated intent、walk/sprint/jump/gravity、voxel collision、stale-input timeout、motion reset 与连续权威状态 source | dedicated-server composition-owned；只写 server-owned player state 和 `IGameAuthoritativePlayerMotionSource`，不读取 client position；连续状态是 online-only，不进入 `SNTP`，且与客户端 prediction 共用 `snt_game_player_motion` 内核 |
| `game/server/game_server_player_death.*` | bed anchor、sidecar grave、death transaction、respawn resolver 和 direct grave collection | dedicated-server composition-owned；墓碑同时是 v13 sidecar record、`CUSTOM` block-entity anchor 和不可破坏 terrain cell；interaction service 与当前 combat service 调用它，网络 payload 不直接改世界 |
| `game/server/game_server_player_combat.*` | 可信 server-side damage、online health 和死亡 producer | dedicated-server composition-owned；`GameWildCreatureSystem` 只提供 account/位置值和 damage request，combat service 先提交墓碑/重生事务再恢复满生命；同一账号每个 fixed tick 只会结算一次死亡，避免陈旧捕食者快照重复命中 |
...
`QuestRegistry` 在相同 main-thread fixed tick 以内容 revision 同步定义，按 stable player/quest key 保存进度；前置满足后自动由 `Locked` 进入 `InProgress`，没有接取状态或脚本 `auto_start` 开关。`quest_progress.h` 将值记录和 `IQuestProgressPersistence` 从 runtime registry 分离，`GameSaveQuestProgressPersistence` 通过 `GameSaveManager` 在显式 universe root 下保存严格的 `SNTQ` v2 per-player 文件；`load_player_progress()` / `save_player_progress()` 只能由玩家生命周期调用。每个玩家的 `QuestRegistry::progress_revision()` 只在持久化值变化时递增，`GameServerPlayerLifecycle` 在 after-fixed-tick barrier 按 `persistence.player_progress_autosave_interval_ticks`（默认 1200）checkpoint 已变化任务进度和被 gameplay 标记 dirty 的 `SNTP` player state；0 禁用定时保存，但断线和关服仍强制写入。world lifecycle 与任务文件互不替代：前者由 simulation session 在启动/关闭处理空间 chunk/sidecar、锚定机器、床和墓碑，后者由 dedicated-server 玩家生命周期按 account 处理。`GameServerQuestEventService` 已在 server composition 中消费已提交的挖矿、方块放置/机器产物领取和带 account 的机器完成事件；它只以主机库存查询刷新 `acquire_item`，并推进自动解锁任务的 `mine_block`/`craft_item`。`SNTG` v12 的专用 `QuestClaimReward` command 在稳定服务端顺序中调用 `claim_reward()`；结构化 item reward 请求完整主机 inventory transaction，失败不写 `reward_claimed`；unlock reward 只修改任务值状态。`GameServerQuestBookReplication` 只为认证账号生成 `QuestBookSnapshot`，`GameServerPlayerReplication` 以 per-observer baseline 与 progress revision 变化产生 value delta，`GameClientQuestBookState` 只读应用该值。MUI 任务书以章节任务图、前置连线、进度和显式领奖消费该缓存，且不提供接取入口。机器 persistence 已通过 `MachineRuntimePersistenceRecord + BlockEntityPlacement(MACHINE)`、稳定 Guid、手动任务 owner 恢复和未锚定拒绝保存完成。save slot 选择仍未实现，不能把该模块当成完整任务玩法。详细迁移顺序见 [p7_玩法迁移设计](p7_玩法迁移设计.md)。
...
- `game/scripts/20_machine_catalog.as`、`30_recipe_catalog.as` 和 `40_quest_catalog.as` 分别拥有机器/放置与激活条件、配方、任务书定义；它们在 `00_material_catalog.as` 与 `10_item_catalog.as` 后按文件名顺序加载。内容重载请求从所选类别向下游展开并在一个脚本事务内提交：materials -> items/machines/recipes/quests，items -> machines/recipes/quests，machines -> recipes。客户端 F5 保留为 `all` 入口，页面可枚举相同 target 列表并选择其中任一类别。自动炉子，以及手动 pit kiln、charcoal pit、bloomery 和 anvil 的配方分别保留旧 pottery 时长、`wood_dust x16 -> charcoal x8`、`iron_crushed x5 + charcoal x5 -> iron_bloom` 和 `iron_bloom -> wrought_iron_ingot`。内容分别声明 pit kiln/charcoal pit 的 cover + ignition、bloomery 的 ignition + valid structure 和 anvil 的 `hammer` tag，并注册 `flint_and_steel` 与 `hammer` 的当前 string id。`MachineStructureValidator` 已从当前 `ChunkRegistry` 验证 bloomery 的 3x3x2/17-cell structure，缺失 chunk 不会被当作成功；认证玩家、库存扣除、方块编辑，以及由 `MachinePlacementRegistry` 映射的已锚定机器放置、activation/collect 已由 player-command interaction 层接入。协作联机路径信任客户端的 machine 提示而不重做该校验；认证图形客户端会从 DDA target 和 remote-machine anchor 值映射输入，已提交方块编辑与 machine anchor 会进入 P7.5 terrain/machine presentation source。生产机器 MUI 已以权威 presentation、`MachineInputSlotTransfer` 和私有库存回执完成双向输入槽编辑。
...
游戏 server 已按“收包 -> 命令稳定排序/应用 -> 权威移动 -> 游戏 fixed tick -> scheduler barrier -> 发包 -> LAN discovery reply”接入 `ReplicationService`。`snt_game_network` 的 `GameServerReplicationHandler` 解析游戏拥有的 `SNTG` v20 envelope：可靠 `LoginRequest` 会先由 `GameAccountPeerAuthenticator` 比较可选的 runtime-injected server password，再经可注入 `IGamePeerAuthenticator` 身份映射，并在 barrier 后发送 `LoginAccepted`；认证后的 `Command` 与唯一 UDP/unreliable `ClientMovementInput` 交给可注入 `IGameReplicationCommandSink`。`GameServerCommandSink` 分别验证 reliable command 和 UDP movement sequence，合并每 peer 最新 movement intent，并把 `QuestClaimReward` 与 `BlockInteraction` 稳定排序；`QuestRegistry` 随后的 fixed tick 依据前置图自动激活任务，已提交玩法事件推进任务。`GameServerPlayerMovement` 通过 `snt_game_player_motion` 在 fixed tick 中执行速度、重力、跳跃、体素碰撞和 stale timeout，随后向 player state 与连续权威状态 source 写回结果。`GameServerPlayerInteractionService` 实现选定的“主机权威、客户端可信提示”模型：客户端提供本地射线、挖掘进度、所选物品和机器 cover/ignition/structure/tool 提示，主机是 terrain、inventory、sidecar 和 persistence-visible mutation 的唯一提交者；它只检查认证会话、同 dimension、基础 reach、已加载 chunk、内容目录、库存非负与 sidecar 完整性，不重放反作弊式 raycast、挖掘时间或机器前置条件。

`GameServerQuestEventService` 只消费上述已提交交互和 `MachineTickSystem` 的完成值事件：挖矿/方块放置/机器产物领取以认证 account 的主机库存刷新 `acquire_item`，挖矿推进 `mine_block`，手动机器完成按 owner account 与产物推进 `craft_item`。事件 producer 的 active-peer/库存查询或进度记录错误以 20 tick 节流 Warn 记录。item reward 只能由 `QuestClaimReward` 专用 command 调用 `QuestRegistry::claim_reward()`，bridge 以一笔主机 inventory transaction 提交全部 stack；无 active peer、库存空间不足或事务失败时记录低频规则拒绝，不设置 `reward_claimed`。`ScienceAndTheologyServerSession` 还组合 `GameServerPlayerBedService`、`GameServerPlayerGraveStore`、`GameServerPlayerRespawnResolver`、`GameServerPlayerDeathService`、`GameServerPlayerCombatService` 和 interaction service：床/墓碑值进入 v13 sidecar，死亡会先创建不可破坏墓碑、再清空非装备背包并重置运动状态；combat service 从 wildlife 的 account/位置快照和 damage value 接口接收捕食者命中，致死时先完成该事务再恢复在线生命；交互已覆盖通用 mine/place、床 use 和已锚定机器 activation/collect。

注入 interest provider 和 snapshot source 时，handler 严格重解析 server-originated Snapshot/Delta、对每 peer 应用 bytes/chunk/entity/value/block budget，并在完整 batch 成功后才加入 outbound。`GameServerPlayerReplication` 读取 server-owned player ECS 值、连续 motion source、已加载 `ChunkRegistry`、chunk-sidecar anchor/live machine runtime 和注入的 current value source，生成包含观察者自身的 player AOI 与独立的 loaded-chunk AOI snapshot。它以 per-observer player/chunk/machine/value baseline 发送 player v2 连续 motion/equipment presentation delta、player AOI remove、RLE terrain chunk enter/exit、已提交 interaction 驱动的 block cell delta、machine presentation upsert/remove 或 account value upsert/remove；静止玩家不会只因 `source_tick` 变化重发 delta，顶号和断线清理旧 peer baseline。图形 client 在 `client_network.enabled` 时以 `GameClientReplicationSession` 组合 TCP+UDP，严格核对 LoginAccepted、发送 movement intent、drain 有序 Snapshot/Delta；该会话已经提供 block-interaction 与 quest-claim queue，`ScienceAndTheologyClientSession` 已将 `J` 快捷键、MUI 任务书、领奖队列与认证后本地 DDA block/machine input 接入。它将 player v2 entity 值应用到无头 `GameRemotePlayerWorld`、terrain 值应用到 `GameClientRemoteChunkWorld`、machine presentation 值应用到 `GameRemoteMachineWorld`、认证账号任务书值应用到 `GameClientQuestBookState`；本账号经过 `GameClientPlayerPrediction` 的 ACK 回放与帧级平滑更新相机，其他账号进入 `GameRemotePlayerInterpolator` 缓存，再由 `GameRemotePlayerPresentationWorld` 每帧采样为 transient full/LOD avatar ECS 实体。remote terrain 覆盖前保存 bootstrap chunk，AOI exit/断线时恢复并交给 `ChunkRenderSystem` 重网格。Steam 请求仍必须经 `ISteamSessionTicketVerifier` 返回 SteamID64；当前 server 未注入该 adapter 时明确拒绝，绝不信任客户端声明的 SteamID。`InterestUpdate`、snapshot acknowledgement、Steamworks backend 和墓碑 UI 仍未实现，不能把该基线误当为完整联机玩法。详情见 [游戏网络协议设计](游戏网络协议设计.md)。
...
7. **无头与网络**：已完成 transport 基线、game account identity、`SNTG` v20 准入、graphical client 的 local-name 登录闭环、可选 runtime server password、原生 IPv4 LAN discovery 与兼容协议 browser、前置自动解锁、显式 `QuestClaimReward`、已提交交互/机器事件的任务 producer、显式 item reward 主机事务、主机权威/客户端可信提示的 `BlockInteraction`、认证图形客户端 DDA 方块/机器交互输入绑定、独立 UDP movement intent、server-owned continuous movement/physics、player v2 continuous replication、本地 prediction/ACK 回放/帧级平滑、远端插值与原生 full/LOD avatar、budgeted snapshot/delta 值同步、concrete player/loaded-chunk/machine AOI/source、认证账号任务书 value source/cache、client remote-player/chunk/machine value world、RLE terrain snapshot、interaction block delta、MUI 任务书与生产机器输入槽编辑、dedicated-server 任务/玩家生命周期与 revision/dirty-gated autosave、client/server 共用的 current-format world load/bootstrap/shutdown-save 生命周期、chunk-sidecar 机器运行态恢复/保存，以及 bed/grave/death/respawn 与 online combat server services。`snt_game_server` 通过 `ScienceAndTheologyServerSession` 可选组合 TCP+UDP、LAN responder、`snt_game_network`、server-owned command sink、movement、player lifecycle、player replication、task-book source、interaction、quest event/reward、sidecar death 和 predator combat services，以 `--ticks N` 运行完整游戏包；`snt_game_identity` 提供 Steam/local-name 名字空间、本地 `SNTI` profile 与首启 prompt contract，`snt_game_tests` 覆盖同名隔离、资料恢复、v20 envelope、独立密码、LAN discovery 过滤、reliable/UDP 独立 sequence、movement/block-interaction/quest-claim codec、snapshot/delta roundtrip、player v2 codec、prediction/interpolation、远端 avatar ECS 投影、provider budget 的原子拒绝、client value drain、task-book codec/revision gate/source、自动任务激活、MUI 任务书、player AOI/continuous motion/equipment/value delta/AOI remove、terrain snapshot/delta/removal/bootstrap restore、machine presentation codec/cache remove、本地同名会话接管、登录确认、localhost TCP+UDP、断线取消、玩家任务/状态重启恢复、dirty autosave、世界重启恢复/损坏存档不覆盖、machine record restore/capture/owner round-trip/未锚定拒绝、bed/grave/death/respawn、捕食者伤害到死亡事务与同 tick 重生保护、任务 event/reward bridge，以及主机 interaction 事务。下一步注入 Steamworks identity/ticket adapter 和墓碑 UI。
...
| game-owned `MachineTickSystem`、reload-safe 配方快照 | 已实现 P7.2.4：多输入槽、稳定 recipe id 选择、显式手动激活以及 `MachineInteractionService` activation context 校验已进入 game simulation；`MachineStructureValidator` 已验证 bloomery 的 3x3x2/17-cell voxel structure；炉子、pit kiln、charcoal pit、bloomery、anvil、flint-and-steel 与 hammer 内容已进入 AngelScript 包；认证玩家、库存、方块编辑，以及由 content registry 映射的已锚定机器放置、activation/collect 已经由主机 interaction service 接入；已提交方块编辑和 sidecar-anchor machine 已接到 P7.5 replication source，认证图形客户端已完成机器交互输入绑定，生产机器 MUI 已通过 `MachineInputSlotTransfer` 和私有库存回执完成双向输入槽编辑 |
| P7.3/P7.4 任务、玩家、世界与机器持久化 | 已实现：结构化 definitions/objective/reward、前置自动解锁/推进/完成/显式领取/repeatable、reload reconciliation、per-player `SNTQ` v2 存取、严格拒绝、backup recovery 与 dedicated-server 认证/顶号/断线/关服 lifecycle、revision-gated autosave；`SNTP` 玩家值状态的加载、断线/关服保存和 dirty-gated checkpoint；共享 session 的 current-format world load/bootstrap/shutdown-save、坏 chunk 不部分加载、v13 chunk-sidecar 的 machine restore/capture/owner round-trip、床 anchor 与不可破坏墓碑记录；已提交的主机交互/机器事件推进任务，item reward 经主机库存事务与 `QuestClaimReward` 显式领取；认证账号 `QuestBookSnapshot` 已复制到客户端只读缓存并由 MUI 任务书显示；捕食者战斗死亡 producer 已接入，墓碑 UI 未实现 |
| 其余玩法迁移和持久状态 | 未完成 |
| 网络 replication | transport、wire codec、主线程 phase 编排、`SNTG` v20、local-name 认证/会话接管、graphical client connection、MUI LAN server list/selection、自动任务图、显式 QuestClaimReward、主机权威/客户端可信提示的 block interaction、认证图形客户端 DDA 机器/方块交互输入、独立 UDP movement intent、server-owned continuous movement/physics、player v2 strict value codec、本地 prediction/ACK 回放/帧级平滑、远端插值与原生 full/LOD avatar、provider budget、concrete player/loaded-chunk/machine interest/source、认证账号 task-book source、client remote-player/chunk/machine/task-book value world、RLE terrain snapshot、interaction block delta、MUI 任务书、生产机器输入槽编辑、`SNTP` lifecycle、bed/grave/death/respawn 与捕食者值伤害到 online combat/death transaction 已实现；Steamworks ticket verifier 和墓碑 UI 未实现 |
