...
>
> 当前工作区包含 `SimulationRuntime + ISimulationSession`、`ClientRuntime + IClientSession`、`snt_game_server`、`snt_network`、`snt_game_identity`、`snt_game_network`、game-owned P7.2.1 机器运行态、P7.3 `QuestRegistry` core、P7.4 task progress provider、dedicated-server 玩家生命周期与共享世界存档生命周期、P0 CMake 依赖隔离和 A 方案 voxel/game world 数据边界变更。
...
存档只读写最新格式：游戏 chunk payload 当前为 v9，engine raw region framing 当前为 v2。格式升级时同步更新测试与开发资产，旧格式直接拒绝，不保留兼容读取或离线转换器。
...
| P1 | 游戏账号身份、`SNTG` v3、登录准入、客户端 local-name 连接闭环、首条权威 QuestAccept 命令和任务存档生命周期已实现；Steamworks 平台 backend、玩家移动/方块编辑、snapshot/delta/AOI 尚不存在，音频尚不存在 | `snt_game_identity` 以 `steam:<SteamID64>` / `local-name:<name>` 隔离账号；`snt_game_network` 已实现 typed login/command codec、会话接管、`GameServerReplicationHandler` 与可注入 transport 的 `GameClientReplicationSession`；server-owned `GameServerCommandSink` 和 `GameServerPlayerLifecycle` 以稳定账号应用 QuestAccept 并在玩家生命周期保存任务，不伪造 Steam 登录或 snapshot/delta 同步 |
...
| `snt_game_world_save` | `GameChunkSerializer`、`GameSaveManager`、planet summary、`SNTQ` player progress provider | 只接受完整 game payload v9；通过 raw v2 region framing 持久化，任务 provider 只交换值类型 |
...
| `snt_game_simulation` | `ScienceAndTheologySimulationSession`、游戏配置、脚本内容、机器 worker、QuestRegistry、world persistence lifecycle、demo terrain/sidecar | client/server 共用且只链接 simulation 闭包；不含 SDL、Vulkan、scene、player 或 UI；会话在 demo bootstrap 前加载 current-format world，并仅在 ready 状态关闭时保存 |
...
| `snt_game_network` | 游戏 `SNTG` v3 envelope、typed login/command codec、client/server 准入 handler、认证/命令/AOI/snapshot contracts | game-owned 且 headless；`GameAccountPeerAuthenticator` 已允许 local-name 并让同 account 新登录接管旧 peer；`GameClientReplicationSession` 可组合任意 transport，当前 TCP+UDP factory 已由 graphical client 驱动；Steam ticket verifier、AOI 和 snapshot source 的 concrete gameplay provider 尚未接入 |
...
| `game/server/game_server_player_lifecycle.*` | 认证玩家任务 load、顶号内存转移、断线/关服 save | dedicated-server composition-owned；只借用 `QuestRegistry` 与 `GameSaveQuestProgressPersistence`，不持有 transport/ECS World；`universe_save_dir` 由 server session 解析到 `user_root`，fixed tick 不做文件 I/O |
...
`ScienceAndTheologySimulationSession::create_world` 在加载脚本、机器 worker 后，先通过 `GameWorldPersistenceLifecycle` 验证并加载 current-format universe/dimension；只有不存在任何 save header 时才 bootstrap demo terrain/sidecar。已识别的空 universe 不 bootstrap，损坏或 seed/mode/dimension 不匹配会失败并保持 `world_ready_ = false`，因此 shutdown 不会覆盖原存档。`ScienceAndTheologyClientSession::create_client_world` 才加载 scene、固定 camera Guid、玩家控制和玩法 UI，并将已生成 chunk 标记为待 mesh。脚本 watcher 轮询已移到 simulation fixed tick，因此图形客户端和无头服务端遵循同一 reload 生命周期；客户端仅保留 F5 的显式 reload 触发。`ScienceAndTheologyServerSession` 组合同一 simulation session：可选 TCP+UDP transport 只在该 wrapper 中创建，`--ticks N` 默认不监听端口，`--network`（及端口覆盖）才打开监听。关闭时 ClientRuntime 先调用 session shutdown；world ready 时 session 先保存当前 dimension/sidecar 再写 universe header，随后停止 SimulationRuntime scheduler/job，随后销毁 render/chunk/UI/GPU assets，最后释放 simulation 的 script、world、catalog/source 和文件日志。这样 worker 不会在 world 或 GPU 资源销毁后继续访问快照结果，AssetManager 也在其借用的 source/catalog 之前释放。CMake 目标依赖已在配置期审计；运行时初始化/关闭顺序仍由手写生命周期维持，尚无通用运行时依赖图自动校验。
...
| World save | game payload v9 + raw region framing v2 | `GameWorldPersistenceLifecycle` 在 session 边界用 `GameSaveManager` 组合 generic voxel chunk 与 game sidecar；仅接受当前格式 |
...
当前实现已无旧格式分支：game-owned `GameChunkSerializer` 只接受完整的 v9 payload，engine-owned `VoxelRegionFile` 只接受 raw region v2 framing，`GameSaveManager` 在一次维度加载中汇总报告被拒绝的 region/chunk 数，`GameWorldPersistenceLifecycle` 还校验 universe/dimension header、seed、mode 和 dimension id，Scene v1 只接受当前已知组件类型。格式升级时直接重生成开发资产和场景。
...
`QuestRegistry` 在相同 main-thread fixed tick 以内容 revision 同步定义，按 stable player/quest key 保存进度。`quest_progress.h` 将值记录和 `IQuestProgressPersistence` 从 runtime registry 分离，`GameSaveQuestProgressPersistence` 通过 `GameSaveManager` 在显式 universe root 下保存严格的 `SNTQ` v1 per-player 文件；`load_player_progress()` / `save_player_progress()` 只能由玩家生命周期调用，fixed tick 不读写文件。world lifecycle 与任务文件互不替代：前者由 simulation session 在启动/关闭处理空间 chunk/sidecar，后者由 dedicated-server 玩家生命周期按 account 处理。玩家/机器事件 producer、reward sink、save slot 选择和机器运行态持久化尚未实现，不能把该模块当成完整任务玩法；机器还缺少空间锚点与 ECS 恢复工厂。详细迁移顺序见 [p7_玩法迁移设计.md](p7_玩法迁移设计.md)。
...
游戏 server 已按“收包 -> 命令稳定排序/应用 -> 游戏 fixed tick -> scheduler barrier -> 发包”接入 `ReplicationService`。`snt_game_network` 的 `GameServerReplicationHandler` 现在解析游戏拥有的 `SNTG` v3 envelope：可靠 `LoginRequest` 经可注入 `IGamePeerAuthenticator` 认证后，在 barrier 后发送 `LoginAccepted`；认证后的 `Command` 交给可注入 `IGameReplicationCommandSink`。默认 `GameAccountPeerAuthenticator` 将 local-name 映射为 `local-name:<validated name>`，同 id 的后登录在 outbound 阶段通过 `IReplicationFrameSink::disconnect()` 接管并断开旧 peer。`ScienceAndTheologyServerSession` 已安装 `GameServerCommandSink` 和 `GameServerPlayerLifecycle`：前者当前只接受 typed `QuestAccept`，按 account id、peer、递增 client sequence 稳定排序后于 `QuestRegistry::tick` 前应用；后者首次认证加载任务进度、顶号只转移内存状态、正常断线和关服保存 resident account，存档根由 `persistence.universe_save_dir` 解析到 `user_root`。图形 client 在 `client_network.enabled` 时以 `GameClientReplicationSession` 组合 TCP+UDP：fixed tick 前 poll inbound、barrier 后 emit outbound，并严格核对 LoginAccepted 的 provider/account id；它提供 `enqueue_quest_accept()`，但当前 UI 尚无任务面板调用该方法。该 session 也接受任意 `IReplicationTransport` 与显式 server peer，以预留 Steam P2P 路径。Steam 请求必须经 `ISteamSessionTicketVerifier` 返回 SteamID64；当前 server 未注入该 adapter 时明确拒绝，绝不信任客户端声明的 SteamID。`InterestUpdate`、snapshot acknowledgement、snapshot/delta producer、Steamworks backend、玩家移动/方块编辑、真实任务 producer/reward 和机器运行态持久化仍未实现，不能把该基线误当为完整联机玩法。详情见 [游戏网络协议设计.md](游戏网络协议设计.md)。
...
7. **无头与网络**：已完成 transport 基线、game account identity、`SNTG` v3 准入、graphical client 的 local-name 登录闭环、首条权威 `QuestAccept`、dedicated-server 任务生命周期，以及 client/server 共用的 current-format world load/bootstrap/shutdown-save 生命周期。`snt_game_server` 通过 `ScienceAndTheologyServerSession` 可选组合 TCP+UDP、`snt_game_network`、server-owned command sink 与 player lifecycle，以 `--ticks N` 运行完整游戏包；`snt_game_identity` 提供 Steam/local-name 名字空间、本地 `SNTI` profile 与首启 prompt contract，`snt_game_tests` 覆盖同名隔离、资料恢复、v3 envelope、本地同名会话接管、登录确认、localhost QuestAccept、sequence 去重、断线取消、真实任务存档重启恢复，以及世界重启恢复/损坏存档不覆盖。下一步注入 Steamworks identity/ticket adapter，再为玩家实体状态确定移动/方块编辑命令、interest、snapshot/delta、真实任务 producer/reward 和以 chunk-sidecar 空间锚定的机器运行态持久化。
...
| 数据定义、世界生成、region 存档 | A 方案首轮已实现；`snt_voxel_data/storage` 只保留通用 terrain/chunk/raw region framing，`snt_game_world_*` 拥有游戏语义与 v9 payload；`GameWorldPersistenceLifecycle` 已在共享 simulation session 中验证/加载/关闭保存当前 dimension，region v2 与 chunk v9 只接受当前格式 |
...
| P7.3/P7.4 任务与世界持久化 | 已实现：结构化 definitions、接取/推进/完成/前置解锁/repeatable、reload reconciliation、per-player `SNTQ` v1 存取、严格拒绝、backup recovery 与 dedicated-server 认证/顶号/断线/关服 lifecycle；共享 session 的 current-format world load/bootstrap/shutdown-save 与损坏不覆盖；真实 producer、reward effect 和机器运行态持久化未实现 |
| 其余玩法迁移和持久状态 | 未完成 |
| 网络 replication | transport、wire codec、主线程 phase 编排、`SNTG` v3、local-name 认证/会话接管、graphical client connection 和 authoritative QuestAccept 已实现；Steamworks ticket verifier、玩家移动/方块编辑、interest、snapshot/delta 未实现 |
