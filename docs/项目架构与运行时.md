...
> 当前架构基线，更新于 2026-07-15。
>
> 当前工作区包含 `SimulationRuntime + ISimulationSession`、`ClientRuntime + IClientSession`、`snt_game_server`、`snt_network`、`snt_game_identity`、`snt_game_network`、game-owned P7.2.4 多输入/手动激活机器运行态、activation context 边界与原始工业内容、P7.3 `QuestRegistry` core、P7.4 task progress provider、dedicated-server 玩家生命周期、共享世界存档生命周期与 chunk-sidecar 机器恢复/保存、P7.5 `SNTG` v4 snapshot/delta 值 codec、budgeted provider scheduling 与 client drain、P0 CMake 依赖隔离和 A 方案 voxel/game world 数据边界变更。
...
存档只读写最新格式：游戏 chunk payload 当前为 v11，engine raw region framing 当前为 v2。格式升级时同步更新测试与开发资产，旧格式直接拒绝，不保留兼容读取或离线转换器。
...
| P1 | 游戏账号身份、`SNTG` v4、登录准入、客户端 local-name 连接闭环、首条权威 QuestAccept 命令、任务存档生命周期和 snapshot/delta 值同步基线已实现；Steamworks 平台 backend、玩家移动/方块编辑、concrete AOI/source 和 remote-world application 尚不存在，音频尚不存在 | `snt_game_identity` 以 `steam:<SteamID64>` / `local-name:<name>` 隔离账号；`snt_game_network` 已实现 typed login/command/snapshot/delta codec、会话接管、budgeted `GameServerReplicationHandler` 与可注入 transport 的 `GameClientReplicationSession` value drain；server-owned `GameServerCommandSink` 和 `GameServerPlayerLifecycle` 以稳定账号应用 QuestAccept 并在玩家生命周期保存任务，production server 不伪造 Steam 登录或世界同步 |
...
flowchart TD
    ...
    GameSave["snt_game_world_save / v11 game payload"] --> GameWorldData
...
| `snt_game_world_save` | `GameChunkSerializer`、`GameSaveManager`、planet summary、`SNTQ` player progress provider | 只接受完整 game payload v11；通过 raw v2 region framing 持久化，任务 provider 只交换值类型 |
...
| `game/quest/quest_registry.*` + `quest_progress.h` | per-player task state、objective progress、reload reconciliation、稳定存档值接口和内存 revision | game-owned、simulation-main-thread-only；定义/进度/效果/持久化边界分离；revision 仅在持久化值变化时递增，真实 player/machine producer 和 reward sink 尚未接入 |
...
| `snt_game_network` | 游戏 `SNTG` v4 envelope、typed login/command/snapshot/delta codec、client/server 准入 handler、认证/命令/AOI/source contracts | game-owned 且 headless；`GameAccountPeerAuthenticator` 已允许 local-name 并让同 account 新登录接管旧 peer；handler 对注入 provider 执行严格 codec + per-peer budget + atomic queue，client 只 drain 值更新；当前 TCP+UDP factory 已由 graphical client 驱动，Steam ticket verifier、concrete AOI/source 和 remote-world consumer 尚未接入 |
| `game/server/game_server_command_sink.*` | `QuestAccept` 的 session sequence、稳定排序、断线取消和任务状态 mutation | dedicated-server composition-owned；只借用 `QuestRegistry`，不持有 transport/ECS World；未知命令是协议拒绝，任务规则拒绝以低频聚合 Warn 记录 |
| `game/server/game_server_player_lifecycle.*` | 认证玩家任务 load、顶号内存转移、断线/关服强制 save 与 revision-gated autosave | dedicated-server composition-owned；只借用 `QuestRegistry` 与 `GameSaveQuestProgressPersistence`，不持有 transport/ECS World；`universe_save_dir` 由 server session 解析到 `user_root`，文件 I/O 仅在 after-fixed-tick barrier 的受控 checkpoint 或生命周期边界发生 |
...
| `snt_game_server` | 运行包定位、配置读取、`ScienceAndTheologyServerSession` 与 TCP+UDP/`snt_game_network`、命令和玩家生命周期组合 | 支持 `--ticks N`、`--network`、`--bind`、`--tcp-port`、`--udp-port`；不链接 SDL/Vulkan/client runtime；local-name 准入可用，任务进度位于 `user/saves/default`（可配置），Steam ticket 无 verifier 时明确拒绝，版本错误、非可靠或未实现 gameplay frame 同样明确拒绝 |
...
| World save | game payload v11 + raw region framing v2 | `GameWorldPersistenceLifecycle` 在 session 边界用 `GameSaveManager` 组合 generic voxel chunk 与 game sidecar；仅接受当前格式 |
...
当前实现已无旧格式分支：game-owned `GameChunkSerializer` 只接受完整的 v11 payload，engine-owned `VoxelRegionFile` 只接受 raw region v2 framing，`GameSaveManager` 在一次维度加载中汇总报告被拒绝的 region/chunk 数并拒绝部分维度加载，`GameWorldPersistenceLifecycle` 还校验 universe/dimension header、seed、mode 和 dimension id，Scene v1 只接受当前已知组件类型。格式升级时直接重生成开发资产和场景。
...
`QuestRegistry` 在相同 main-thread fixed tick 以内容 revision 同步定义，按 stable player/quest key 保存进度。`quest_progress.h` 将值记录和 `IQuestProgressPersistence` 从 runtime registry 分离，`GameSaveQuestProgressPersistence` 通过 `GameSaveManager` 在显式 universe root 下保存严格的 `SNTQ` v1 per-player 文件；`load_player_progress()` / `save_player_progress()` 只能由玩家生命周期调用。每个玩家的 `QuestRegistry::progress_revision()` 只在持久化值变化时递增，`GameServerPlayerLifecycle` 在 after-fixed-tick barrier 按 `persistence.player_progress_autosave_interval_ticks`（默认 1200）只 checkpoint 已变化的 resident account；0 禁用定时保存，但断线和关服仍强制写入。world lifecycle 与任务文件互不替代：前者由 simulation session 在启动/关闭处理空间 chunk/sidecar 与锚定机器，后者由 dedicated-server 玩家生命周期按 account 处理。机器 persistence 已通过 `MachineRuntimePersistenceRecord + BlockEntityPlacement(MACHINE)`、稳定 Guid 恢复和未锚定拒绝保存完成；玩家/机器事件 producer、reward sink 和 save slot 选择仍未实现，不能把该模块当成完整任务玩法。详细迁移顺序见 [p7_玩法迁移设计.md](p7_玩法迁移设计.md)。
...
游戏 server 已按“收包 -> 命令稳定排序/应用 -> 游戏 fixed tick -> scheduler barrier -> 发包”接入 `ReplicationService`。`snt_game_network` 的 `GameServerReplicationHandler` 现在解析游戏拥有的 `SNTG` v4 envelope：可靠 `LoginRequest` 经可注入 `IGamePeerAuthenticator` 认证后，在 barrier 后发送 `LoginAccepted`；认证后的 `Command` 交给可注入 `IGameReplicationCommandSink`。当同时注入 interest provider 和 snapshot source 时，handler 还会严格重解析 server-originated Snapshot/Delta、对每 peer 应用 bytes/chunk/entity/block budget，并在完整 batch 成功后才加入 outbound；未发送同步帧会阻止继续生产 delta。默认 `GameAccountPeerAuthenticator` 将 local-name 映射为 `local-name:<validated name>`，同 id 的后登录在 outbound 阶段通过 `IReplicationFrameSink::disconnect()` 接管并断开旧 peer。`ScienceAndTheologyServerSession` 已安装 `GameServerCommandSink` 和 `GameServerPlayerLifecycle`：前者当前只接受 typed `QuestAccept`，按 account id、peer、递增 client sequence 稳定排序后于 `QuestRegistry::tick` 前应用；后者首次认证加载任务进度、顶号只转移内存状态、正常断线和关服保存 resident account，并在 barrier 按配置间隔只保存 value revision 已变化的账号，存档根由 `persistence.universe_save_dir` 解析到 `user_root`。图形 client 在 `client_network.enabled` 时以 `GameClientReplicationSession` 组合 TCP+UDP：fixed tick 前 poll inbound、barrier 后 emit outbound，并严格核对 LoginAccepted 的 provider/account id；认证后的 snapshot/delta 只以有界、顺序的值 update drain 给宿主，未创建 remote World。该 session 也接受任意 `IReplicationTransport` 与显式 server peer，以预留 Steam P2P 路径。Steam 请求必须经 `ISteamSessionTicketVerifier` 返回 SteamID64；当前 server 未注入该 adapter 时明确拒绝，绝不信任客户端声明的 SteamID。`InterestUpdate`、snapshot acknowledgement、concrete AOI/source、remote-world application、Steamworks backend、玩家移动/方块编辑和真实任务 producer/reward 仍未实现，不能把该基线误当为完整联机玩法。详情见 [游戏网络协议设计.md](游戏网络协议设计.md)。
...
6. **最新格式策略**：已完成。`GameChunkSerializer` 只接受完整 v11 payload，`VoxelRegionFile` 只接受 v2，删除 legacy region 探测；Scene v1 拒绝未知组件而不跳过。`GameSaveManager` 以每次维度加载一条聚合 Warn 报告被拒绝的文件或 chunk，并拒绝部分维度加载，开发期格式升级时重生成资产。
7. **无头与网络**：已完成 transport 基线、game account identity、`SNTG` v4 准入、graphical client 的 local-name 登录闭环、首条权威 `QuestAccept`、budgeted snapshot/delta 值同步基线、dedicated-server 任务生命周期与 revision-gated autosave、client/server 共用的 current-format world load/bootstrap/shutdown-save 生命周期，以及 chunk-sidecar 机器运行态恢复/保存。`snt_game_server` 通过 `ScienceAndTheologyServerSession` 可选组合 TCP+UDP、`snt_game_network`、server-owned command sink 与 player lifecycle，以 `--ticks N` 运行完整游戏包；`snt_game_identity` 提供 Steam/local-name 名字空间、本地 `SNTI` profile 与首启 prompt contract，`snt_game_tests` 覆盖同名隔离、资料恢复、v4 envelope、snapshot/delta roundtrip、provider budget 的原子拒绝、client value drain、本地同名会话接管、登录确认、localhost QuestAccept、sequence 去重、断线取消、真实任务存档重启恢复、自动存档间隔门控、世界重启恢复/损坏存档不覆盖，以及 machine record restore/capture/未锚定拒绝。下一步注入 Steamworks identity/ticket adapter，再为玩家实体状态实现 concrete AOI/source、remote-world application、移动/方块编辑命令和真实任务 producer/reward。
...
| 数据定义、世界生成、region 存档 | A 方案首轮已实现；`snt_voxel_data/storage` 只保留通用 terrain/chunk/raw region framing，`snt_game_world_*` 拥有游戏语义与 v11 payload；`GameWorldPersistenceLifecycle` 已在共享 simulation session 中验证/加载/关闭保存当前 dimension，region v2 与 chunk v11 只接受当前格式 |
| voxel meshing、chunk 渲染、玩家碰撞/射线 | 已实现 |
| 保留模式 UI、Unicode shaping/raster | 已实现基础能力 |
| AngelScript 加载、watch、事务热重载 | 已实现 P7.1 |
| game-owned `MachineTickSystem`、reload-safe 配方快照 | 已实现 P7.2.4：多输入槽、稳定 recipe id 选择、显式手动激活以及 `MachineInteractionService` activation context 校验已进入 game simulation；`MachineStructureValidator` 已验证 bloomery 的 3x3x2/17-cell voxel structure；炉子、pit kiln、charcoal pit、bloomery、anvil 内容已进入 AngelScript 包；认证玩家、库存、方块编辑和收集仍待迁移 |
| P7.3/P7.4 任务、世界与机器持久化 | 已实现：结构化 definitions、接取/推进/完成/前置解锁/repeatable、reload reconciliation、per-player `SNTQ` v1 存取、严格拒绝、backup recovery 与 dedicated-server 认证/顶号/断线/关服 lifecycle、revision-gated autosave；共享 session 的 current-format world load/bootstrap/shutdown-save、坏 chunk 不部分加载、chunk-anchored machine restore/capture 与未锚定拒绝保存；真实 producer 和 reward effect 未实现 |
| 其余玩法迁移和持久状态 | 未完成 |
| 网络 replication | transport、wire codec、主线程 phase 编排、`SNTG` v4、local-name 认证/会话接管、graphical client connection、authoritative QuestAccept、snapshot/delta strict value codec、provider budget 和 client drain 已实现；Steamworks ticket verifier、玩家移动/方块编辑、concrete interest/source、remote-world application 未实现 |
