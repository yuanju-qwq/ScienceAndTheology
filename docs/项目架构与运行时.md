...
| P1 | 游戏账号身份、`SNTG` v20、登录准入、客户端 local-name 连接闭环、前置自动解锁与显式 QuestClaimReward、已提交任务 event/reward bridge、主机权威/客户端可信提示的 BlockInteraction、认证图形客户端 DDA 交互绑定、独立 UDP movement intent、任务/玩家存档生命周期、player/loaded-chunk/machine AOI snapshot/delta、player v2 连续复制、本地 prediction/ACK 回放/帧级平滑、远端插值缓存、认证账号任务书只读值同步、MUI 任务书和生产机器输入槽编辑已实现；Steamworks 平台 backend、remote avatar renderer、战斗 death producer 和墓碑 UI 尚未实现，音频尚不存在 | `snt_game_identity` 以 `steam:<SteamID64>` / `local-name:<name>` 隔离账号；`snt_game_network` 已实现 typed login/command/movement/snapshot/delta codec、会话接管、budgeted `GameServerReplicationHandler` 与可注入 transport 的 `GameClientReplicationSession` value drain；`snt_game_player_motion` 让 `GameServerPlayerMovement` 与客户端预测共用碰撞、跳跃和重力规则，`GameServerPlayerReplication` 从 server ECS、连续 motion source、loaded `ChunkRegistry`、sidecar anchor/live machine runtime 和 typed value source 生成四类 AOI/delta baseline；`GameRemotePlayerWorld`、`GameClientRemoteChunkWorld`、`GameRemoteMachineWorld` 与 `GameClientQuestBookState` 应用 presentation-only 值，`GameClientPlayerPrediction` 与 `GameRemotePlayerInterpolator` 将其中的 player v2 值交给相机和未来 avatar renderer；server composition 还拥有 sidecar bed/grave/respawn/death、block/machine interaction 与 quest event/reward services，战斗 death producer 尚未接入 |
...
| `snt_game_network` | 游戏 `SNTG` v20 envelope、独立可选服务器口令、typed login/command/unreliable movement/snapshot/delta codec、兼容协议 LAN browser、client/server 准入 handler、认证/命令/AOI/source contracts、typed account value、player v2、RLE terrain 与 machine v1 presentation codec | game-owned 且 headless；`GameAccountPeerAuthenticator` 已允许 local-name 并让同 account 新登录接管旧 peer，且可在身份映射前验证 runtime-injected server password；handler 对注入 provider 执行严格 codec + bytes/chunk/entity/value/block per-peer budget + atomic queue，`GameClientReplicationSession` 为 reliable command 和 UDP movement 分别维护递增 sequence，`GameLanDiscoveryClient` 只输出当前协议服务器，图形客户端 `LanServerBrowserModel` 与直接 MUI 服务器列表在选择后才建立 transport，`GameRemotePlayerWorld`、`GameClientRemoteChunkWorld`、`GameRemoteMachineWorld` 与 `GameClientQuestBookState` 只应用有序 presentation value；Steam ticket verifier、remote avatar rendering 尚未接入 |
| `snt_game_player_motion` + `game/client/player_motion_presentation.*` | 服务端与客户端共用的连续玩家移动内核，以及本地 prediction/reconciliation 和远端插值值缓存 | shared motion target 只依赖 typed movement、窄 voxel collision view 和 game replication value；`GameServerPlayerMovement` 是唯一权威写入者，客户端只预测已提交输入并以 player v2 ACK 回放；presentation 模块不持有 transport、renderer、ECS World 或持久状态，remote avatar renderer 后续只消费 `GameRemotePlayerInterpolator` 的采样值 |
...
| 其余玩法迁移和持久状态 | 未完成 |
| 网络 replication | transport、wire codec、主线程 phase 编排、`SNTG` v20、local-name 认证/会话接管、graphical client connection、MUI LAN server list/selection、自动任务图、显式 QuestClaimReward、主机权威/客户端可信提示的 block interaction、认证图形客户端 DDA 机器/方块交互输入、独立 UDP movement intent、server-owned continuous movement/physics、player v2 strict value codec、本地 prediction/ACK 回放/帧级平滑、远端插值缓存、provider budget、concrete player/loaded-chunk/machine interest/source、认证账号 task-book source、client remote-player/chunk/machine/task-book value world、RLE terrain snapshot、interaction block delta、MUI 任务书、生产机器输入槽编辑、`SNTP` lifecycle 与 bed/grave/death/respawn server services 已实现；Steamworks ticket verifier、remote avatar renderer、战斗死亡 producer 和墓碑 UI 未实现 |
