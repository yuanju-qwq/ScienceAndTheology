...
> 当前架构基线，更新于 2026-07-18。
>
> 当前工作区包含 `SimulationRuntime + ISimulationSession`、`ClientRuntime + IClientSession`、`snt_game_server`、`snt_network`、`snt_game_identity`、`snt_game_network`、game-owned P7.2.4 多输入/手动激活机器运行态、activation context 边界与原始工业内容、P7.3 `QuestRegistry`、Better Questing 式前置自动解锁、结构化 reward 与 committed event/reward bridge、P7.4 task progress provider、dedicated-server 玩家生命周期、共享世界存档生命周期与 chunk-sidecar 机器恢复/保存、P7.5 `SNTG` v12 snapshot/delta/movement/block-interaction/quest-claim/typed account-value codec、可选服务器口令、原生 IPv4 LAN discovery、budgeted provider scheduling、player/loaded-chunk/machine concrete AOI/source、server-owned movement、RLE terrain payload、只读任务书与机器缓存、图形客户端 remesh handoff，以及主机权威/客户端可信提示的方块、机器和任务奖励领取、sidecar 床/墓碑/重生服务、P0 CMake 依赖隔离和 A 方案 voxel/game world 数据边界变更。
...
| P1 | 游戏账号身份、`SNTG` v12、登录准入、客户端 local-name 连接闭环、前置自动解锁与显式 QuestClaimReward、已提交任务 event/reward bridge、主机权威/客户端可信提示的 BlockInteraction、认证图形客户端 DDA 交互绑定、UDP movement intent、任务/玩家存档生命周期、player/loaded-chunk/machine AOI snapshot/delta、remote player/chunk/machine value world、认证账号任务书只读值同步和 MUI 任务书已实现；网络背包/槽位增量事务已完成；Steamworks 平台 backend、remote avatar rendering、生产机器 UI 和机器输入槽编辑尚未实现，音频尚不存在 | `snt_game_identity` 以 `steam:<SteamID64>` / `local-name:<name>` 隔离账号；`snt_game_network` 已实现 typed login/command/movement/snapshot/delta codec、会话接管、budgeted `GameServerReplicationHandler` 与可注入 transport 的 `GameClientReplicationSession` value drain；`GameServerPlayerMovement` 以输入、碰撞和 gravity 产生权威位置，`GameServerPlayerReplication` 从 server ECS、loaded `ChunkRegistry`、sidecar anchor/live machine runtime 和 typed value source 生成四类 AOI/delta baseline，`GameRemotePlayerWorld`、`GameClientRemoteChunkWorld`、`GameRemoteMachineWorld` 与 `GameClientQuestBookState` 应用 presentation-only 值；server composition 还拥有 sidecar bed/grave/respawn/death、block/machine interaction 与 quest event/reward services，战斗 death producer 尚未接入 |
...
| `snt_game_network` | 游戏 `SNTG` v12 envelope、独立可选服务器口令、typed login/command/unreliable movement/snapshot/delta codec、兼容协议 LAN browser、client/server 准入 handler、认证/命令/AOI/source contracts、typed account value、player v1、RLE terrain 与 machine v1 presentation codec | game-owned 且 headless；`GameAccountPeerAuthenticator` 已允许 local-name 并让同 account 新登录接管旧 peer，且可在身份映射前验证 runtime-injected server password；handler 对注入 provider 执行严格 codec + bytes/chunk/entity/value/block per-peer budget + atomic queue，`GameClientReplicationSession` 以 shared sequence 发送 reward-claim/block-interaction command 和 movement，`GameLanDiscoveryClient` 只输出当前协议服务器，图形客户端 `LanServerBrowserModel` 与直接 MUI 服务器列表在选择后才建立 transport，`GameRemotePlayerWorld`、`GameClientRemoteChunkWorld`、`GameRemoteMachineWorld` 与 `GameClientQuestBookState` 只应用有序 presentation value；Steam ticket verifier、remote avatar rendering 尚未接入 |
...
游戏 server 已按“收包 -> 命令稳定排序/应用 -> 权威移动 -> 游戏 fixed tick -> scheduler barrier -> 发包 -> LAN discovery reply”接入 `ReplicationService`。`snt_game_network` 的 `GameServerReplicationHandler` 解析游戏拥有的 `SNTG` v12 envelope：可靠 `LoginRequest` 会先由 `GameAccountPeerAuthenticator` 比较可选的 runtime-injected server password，再经可注入 `IGamePeerAuthenticator` 身份映射，并在 barrier 后发送 `LoginAccepted`；认证后的 `Command` 与唯一 UDP/unreliable `ClientMovementInput` 交给可注入 `IGameReplicationCommandSink`。`GameServerCommandSink` 验证 shared sequence，合并每 peer 最新 movement intent，并把 `QuestClaimReward` 与 `BlockInteraction` 稳定排序；`QuestRegistry` 随后的 fixed tick 依据前置图自动激活任务，已提交玩法事件推进任务。`GameServerPlayerMovement` 在 fixed tick 中执行速度、重力、跳跃、体素碰撞和 stale timeout，随后把权威 block-grid position 写回 player state。`GameServerPlayerInteractionService` 实现选定的“主机权威、客户端可信提示”模型：客户端提供本地射线、挖掘进度、所选物品和机器 cover/ignition/structure/tool 提示，主机是 terrain、inventory、sidecar 和 persistence-visible mutation 的唯一提交者；它只检查认证会话、同 dimension、基础 reach、已加载 chunk、内容目录、库存非负与 sidecar 完整性，不重放反作弊式 raycast、挖掘时间或机器前置条件。
...
注入 interest provider 和 snapshot source 时，handler 严格重解析 server-originated Snapshot/Delta、对每 peer 应用 bytes/chunk/entity/value/block budget，并在完整 batch 成功后才加入 outbound。`GameServerPlayerReplication` 读取 server-owned player ECS 值、已加载 `ChunkRegistry`、chunk-sidecar anchor/live machine runtime 和注入的 current value source，生成包含观察者自身的 player AOI 与独立的 loaded-chunk AOI snapshot。它以 per-observer player/chunk/machine/value baseline 发送 position/equipment presentation delta、player AOI remove、RLE terrain chunk enter/exit、已提交 interaction 驱动的 block cell delta、machine presentation upsert/remove 或 account value upsert/remove；顶号和断线清理旧 peer baseline。图形 client 在 `client_network.enabled` 时以 `GameClientReplicationSession` 组合 TCP+UDP，严格核对 LoginAccepted、发送 movement intent、drain 有序 Snapshot/Delta；该会话已经提供 block-interaction 与 quest-claim queue，`ScienceAndTheologyClientSession` 已将 `J` 快捷键、MUI 任务书、领奖队列与认证后本地 DDA block/machine input 接入。它将 player v1 entity 值应用到无头 `GameRemotePlayerWorld`、terrain 值应用到 `GameClientRemoteChunkWorld`、machine presentation 值应用到 `GameRemoteMachineWorld`、认证账号任务书值应用到 `GameClientQuestBookState`，并把自身权威格坐标写到相机；remote terrain 覆盖前保存 bootstrap chunk，AOI exit/断线时恢复并交给 `ChunkRenderSystem` 重网格。Steam 请求仍必须经 `ISteamSessionTicketVerifier` 返回 SteamID64；当前 server 未注入该 adapter 时明确拒绝，绝不信任客户端声明的 SteamID。`InterestUpdate`、snapshot acknowledgement、Steamworks backend、生产机器 UI、机器输入槽编辑、remote avatar rendering、客户端插值/reconciliation、战斗死亡 producer 和墓碑 UI 仍未实现，不能把该基线误当为完整联机玩法。详情见 [游戏网络协议设计](游戏网络协议设计.md)。
...
7. **无头与网络**：已完成 transport 基线、game account identity、`SNTG` v12 准入、graphical client 的 local-name 登录闭环、可选 runtime server password、原生 IPv4 LAN discovery 与兼容协议 browser、前置自动解锁、显式 `QuestClaimReward`、已提交交互/机器事件的任务 producer、显式 item reward 主机事务、主机权威/客户端可信提示的 `BlockInteraction`、认证图形客户端 DDA 方块/机器交互输入绑定、UDP movement intent、server-owned movement/physics、budgeted snapshot/delta 值同步、concrete player/loaded-chunk/machine AOI/source、认证账号任务书 value source/cache、client remote-player/chunk/machine value world、RLE terrain snapshot、interaction block delta、MUI 任务书、dedicated-server 任务/玩家生命周期与 revision/dirty-gated autosave、client/server 共用的 current-format world load/bootstrap/shutdown-save 生命周期、chunk-sidecar 机器运行态恢复/保存，以及 bed/grave/death/respawn server services。`snt_game_server` 通过 `ScienceAndTheologyServerSession` 可选组合 TCP+UDP、LAN responder、`snt_game_network`、server-owned command sink、movement、player lifecycle、player replication、task-book source、interaction、quest event/reward 和 sidecar death services，以 `--ticks N` 运行完整游戏包；`snt_game_identity` 提供 Steam/local-name 名字空间、本地 `SNTI` profile 与首启 prompt contract，`snt_game_tests` 覆盖同名隔离、资料恢复、v12 envelope、独立密码、LAN discovery 过滤、movement/block-interaction/quest-claim codec、snapshot/delta roundtrip、provider budget 的原子拒绝、client value drain、task-book codec/revision gate/source、自动任务激活、MUI 任务书、player AOI/position/equipment/value delta/AOI remove、terrain snapshot/delta/removal/bootstrap restore、machine presentation codec/cache remove、本地同名会话接管、登录确认、localhost TCP+UDP、sequence 去重、断线取消、玩家任务/状态重启恢复、dirty autosave、世界重启恢复/损坏存档不覆盖、machine record restore/capture/owner round-trip/未锚定拒绝、bed/grave/death/respawn、任务 event/reward bridge，以及主机 interaction 事务。下一步注入 Steamworks identity/ticket adapter，再实现 LAN browser UI、remote avatar rendering、客户端插值/reconciliation、生产机器 UI、机器输入槽编辑、战斗死亡 producer 和墓碑 UI。
...
| 其余玩法迁移和持久状态 | 未完成 |
| 网络 replication | transport、wire codec、主线程 phase 编排、`SNTG` v12、local-name 认证/会话接管、graphical client connection、MUI LAN server list/selection、自动任务图、显式 QuestClaimReward、主机权威/客户端可信提示的 block interaction、认证图形客户端 DDA 机器/方块交互输入、UDP movement intent、server-owned movement/physics、snapshot/delta strict value codec、provider budget、concrete player/loaded-chunk/machine interest/source、认证账号 task-book source、client remote-player/chunk/machine/task-book value world、RLE terrain snapshot、interaction block delta、MUI 任务书、`SNTP` lifecycle 与 bed/grave/death/respawn server services 已实现；Steamworks ticket verifier、生产机器 UI、机器输入槽编辑、战斗死亡 producer、墓碑 UI、客户端 prediction/reconciliation 与 remote avatar rendering 未实现 |
