...
>
> 当前工作区包含 `SimulationRuntime + ISimulationSession`、`ClientRuntime + IClientSession`、`snt_game_server`、`snt_network`、`snt_game_identity`、`snt_game_network`、game-owned P7.2.1 机器运行态、P7.3 `QuestRegistry` core、P7.4 task progress provider、dedicated-server 玩家生命周期、共享世界存档生命周期与 chunk-sidecar 机器恢复/保存、P0 CMake 依赖隔离和 A 方案 voxel/game world 数据边界变更。
...
存档只读写最新格式：游戏 chunk payload 当前为 v10，engine raw region framing 当前为 v2。格式升级时同步更新测试与开发资产，旧格式直接拒绝，不保留兼容读取或离线转换器。
...
flowchart TD
    ...
    GameSave["snt_game_world_save / v10 game payload"] --> GameWorldData
...
| `snt_game_world_save` | `GameChunkSerializer`、`GameSaveManager`、planet summary、`SNTQ` player progress provider | 只接受完整 game payload v10；通过 raw v2 region framing 持久化，任务 provider 只交换值类型 |
...
| World save | game payload v10 + raw region framing v2 | `GameWorldPersistenceLifecycle` 在 session 边界用 `GameSaveManager` 组合 generic voxel chunk 与 game sidecar；仅接受当前格式 |
...
当前实现已无旧格式分支：game-owned `GameChunkSerializer` 只接受完整的 v10 payload，engine-owned `VoxelRegionFile` 只接受 raw region v2 framing，`GameSaveManager` 在一次维度加载中汇总报告被拒绝的 region/chunk 数并拒绝部分维度加载，`GameWorldPersistenceLifecycle` 还校验 universe/dimension header、seed、mode 和 dimension id，Scene v1 只接受当前已知组件类型。格式升级时直接重生成开发资产和场景。
...
6. **最新格式策略**：已完成。`GameChunkSerializer` 只接受完整 v10 payload，`VoxelRegionFile` 只接受 v2，删除 legacy region 探测；Scene v1 拒绝未知组件而不跳过。`GameSaveManager` 以每次维度加载一条聚合 Warn 报告被拒绝的文件或 chunk，并拒绝部分维度加载，开发期格式升级时重生成资产。
7. **无头与网络**：已完成 transport 基线、game account identity、`SNTG` v3 准入、graphical client 的 local-name 登录闭环、首条权威 `QuestAccept`、dedicated-server 任务生命周期与 revision-gated autosave、client/server 共用的 current-format world load/bootstrap/shutdown-save 生命周期，以及 chunk-sidecar 机器运行态恢复/保存。`snt_game_server` 通过 `ScienceAndTheologyServerSession` 可选组合 TCP+UDP、`snt_game_network`、server-owned command sink 与 player lifecycle，以 `--ticks N` 运行完整游戏包；`snt_game_identity` 提供 Steam/local-name 名字空间、本地 `SNTI` profile 与首启 prompt contract，`snt_game_tests` 覆盖同名隔离、资料恢复、v3 envelope、本地同名会话接管、登录确认、localhost QuestAccept、sequence 去重、断线取消、真实任务存档重启恢复、自动存档间隔门控、世界重启恢复/损坏存档不覆盖，以及 machine record restore/capture/未锚定拒绝。下一步注入 Steamworks identity/ticket adapter，再为玩家实体状态确定移动/方块编辑命令、interest、snapshot/delta 和真实任务 producer/reward。
...
| 数据定义、世界生成、region 存档 | A 方案首轮已实现；`snt_voxel_data/storage` 只保留通用 terrain/chunk/raw region framing，`snt_game_world_*` 拥有游戏语义与 v10 payload；`GameWorldPersistenceLifecycle` 已在共享 simulation session 中验证/加载/关闭保存当前 dimension，region v2 与 chunk v10 只接受当前格式 |
