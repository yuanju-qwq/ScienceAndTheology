...
存档只读写最新格式：游戏 chunk payload 当前为 v12，engine raw region framing 当前为 v2。格式升级时同步更新测试与开发资产，旧格式直接拒绝，不保留兼容读取或离线转换器。
...
| P1 | 游戏账号身份、`SNTG` v5、登录准入、客户端 local-name 连接闭环、权威 QuestAccept、UDP movement intent、任务/玩家存档生命周期、player AOI snapshot/delta 与 remote-player value world 已实现；Steamworks 平台 backend、方块编辑、chunk/machine source、remote avatar rendering 尚不存在，音频尚不存在 | `snt_game_identity` 以 `steam:<SteamID64>` / `local-name:<name>` 隔离账号；`snt_game_network` 已实现 typed login/command/movement/snapshot/delta codec、会话接管、budgeted `GameServerReplicationHandler` 与可注入 transport 的 `GameClientReplicationSession` value drain；`GameServerPlayerMovement` 以输入、碰撞和 gravity 产生权威位置，`GameServerPlayerReplication` 从 server ECS 值生成 AOI/delta baseline，`GameRemotePlayerWorld` 应用 presentation-only player value；server composition 还拥有 sidecar bed/grave/respawn/death services，尚未由战斗或方块编辑命令触发 |
...
flowchart TD
    ...
    GameSave["snt_game_world_save / v12 game payload"] --> GameWorldData
...
| `snt_game_world_save` | `GameChunkSerializer`、`GameSaveManager`、planet summary、`SNTQ`/`SNTP` player provider | 只接受完整 game payload v12；通过 raw v2 region framing 持久化，任务和 player provider 只交换值类型；v12 sidecar 同时保存床 anchor 与墓碑 contents |
...
| `snt_game_network` | 游戏 `SNTG` v5 envelope、typed login/command/unreliable movement/snapshot/delta codec、client/server 准入 handler、认证/命令/AOI/source contracts 与 player v1 value codec | game-owned 且 headless；`GameAccountPeerAuthenticator` 已允许 local-name 并让同 account 新登录接管旧 peer；handler 对注入 provider 执行严格 codec + per-peer budget + atomic queue，`GameClientReplicationSession` 以 shared sequence 发送 command/movement，`GameRemotePlayerWorld` 只应用有序 player value；Steam ticket verifier、chunk/machine source、remote avatar rendering 尚未接入 |
...
| `game/server/game_server_command_sink.*` | `QuestAccept` 的 shared session sequence、稳定排序、latest movement intent 合并、断线取消和任务状态 mutation | dedicated-server composition-owned；只借用 `QuestRegistry` 与 narrow movement input sink，不持有 transport/ECS World；未知命令是协议拒绝，任务规则拒绝以低频聚合 Warn 记录 |
...
| `game/server/game_server_player_death.*` | bed anchor、sidecar grave、death transaction、respawn resolver 和 direct grave collection | dedicated-server composition-owned；墓碑同时是 v12 sidecar record、`CUSTOM` block-entity anchor 和不可破坏 terrain cell；未来方块编辑/战斗系统调用它，不让 network payload 直接改变世界 |
...
| World save | game payload v12 + raw region framing v2 | `GameWorldPersistenceLifecycle` 在 session 边界用 `GameSaveManager` 组合 generic voxel chunk 与 game sidecar；仅接受当前格式 |
...
当前实现已无旧格式分支：game-owned `GameChunkSerializer` 只接受完整的 v12 payload（含机器 runtime、床 anchor 和墓碑 contents），engine-owned `VoxelRegionFile` 只接受 raw region v2 framing，`GameSaveManager` 在一次维度加载中汇总报告被拒绝的 region/chunk 数并拒绝部分维度加载，`GameWorldPersistenceLifecycle` 还校验 universe/dimension header、seed、mode 和 dimension id，Scene v1 只接受当前已知组件类型。格式升级时直接重生成开发资产和场景。
...
游戏 server 已按“收包 -> 命令稳定排序/应用 -> 权威移动 -> 游戏 fixed tick -> scheduler barrier -> 发包”接入 `ReplicationService`。`snt_game_network` 的 `GameServerReplicationHandler` 解析游戏拥有的 `SNTG` v5 envelope：可靠 `LoginRequest` 经可注入 `IGamePeerAuthenticator` 认证后，在 barrier 后发送 `LoginAccepted`；认证后的 `Command` 与唯一 UDP/unreliable `ClientMovementInput` 交给可注入 `IGameReplicationCommandSink`。`GameServerCommandSink` 验证 shared sequence，合并每 peer 最新 movement intent；`GameServerPlayerMovement` 在 fixed tick 中执行速度、重力、跳跃、体素碰撞和 stale timeout，随后把权威 block-grid position 写回 player state。注入 interest provider 和 snapshot source 时，handler 严格重解析 server-originated Snapshot/Delta、对每 peer 应用 bytes/chunk/entity/block budget，并在完整 batch 成功后才加入 outbound。`ScienceAndTheologyServerSession` 还组合 `GameServerPlayerBedService`、`GameServerPlayerGraveStore`、`GameServerPlayerRespawnResolver` 和 `GameServerPlayerDeathService`：床/墓碑值进入 v12 sidecar，死亡会先创建不可破坏墓碑、再清空非装备背包并重置运动状态；当前尚无战斗死亡 producer、方块编辑入口或墓碑 UI。`GameServerPlayerReplication` 只读取 server-owned player ECS 值，生成包含观察者自身的同 dimension AOI snapshot，并以 per-observer baseline 发送 position/equipment presentation delta 或 AOI remove；顶号和断线清理旧 peer baseline。图形 client 在 `client_network.enabled` 时以 `GameClientReplicationSession` 组合 TCP+UDP，严格核对 LoginAccepted、发送 intent、drain 有序 Snapshot/Delta；`ScienceAndTheologyClientSession` 将 player v1 entity 值应用到无头 `GameRemotePlayerWorld`，并把自身权威格坐标写到相机。Steam 请求仍必须经 `ISteamSessionTicketVerifier` 返回 SteamID64；当前 server 未注入该 adapter 时明确拒绝，绝不信任客户端声明的 SteamID。`InterestUpdate`、snapshot acknowledgement、Steamworks backend、玩家方块编辑、chunk/machine source、remote avatar rendering、客户端插值/reconciliation 和真实任务 producer/reward 仍未实现，不能把该基线误当为完整联机玩法。详情见 [游戏网络协议设计.md](游戏网络协议设计.md)。
...
6. **最新格式策略**：已完成。`GameChunkSerializer` 只接受完整 v12 payload，`VoxelRegionFile` 只接受 v2，删除 legacy region 探测；Scene v1 拒绝未知组件而不跳过。`GameSaveManager` 以每次维度加载一条聚合 Warn 报告被拒绝的文件或 chunk，并拒绝部分维度加载，开发期格式升级时重生成资产。
7. **无头与网络**：已完成 transport 基线、game account identity、`SNTG` v5 准入、graphical client 的 local-name 登录闭环、权威 `QuestAccept`、UDP movement intent、server-owned movement/physics、budgeted snapshot/delta 值同步、concrete player AOI/source、client remote-player value world、dedicated-server 任务/玩家生命周期与 revision/dirty-gated autosave、client/server 共用的 current-format world load/bootstrap/shutdown-save 生命周期、chunk-sidecar 机器运行态恢复/保存，以及 bed/grave/death/respawn server services。`snt_game_server` 通过 `ScienceAndTheologyServerSession` 可选组合 TCP+UDP、`snt_game_network`、server-owned command sink、movement、player lifecycle、player replication 和 sidecar death services，以 `--ticks N` 运行完整游戏包；`snt_game_identity` 提供 Steam/local-name 名字空间、本地 `SNTI` profile 与首启 prompt contract，`snt_game_tests` 覆盖同名隔离、资料恢复、v5 envelope、movement codec/channel、snapshot/delta roundtrip、provider budget 的原子拒绝、client value drain、player AOI/position/equipment delta/AOI remove、本地同名会话接管、登录确认、localhost TCP+UDP、sequence 去重、断线取消、玩家任务/状态重启恢复、dirty autosave、世界重启恢复/损坏存档不覆盖、machine record restore/capture/未锚定拒绝以及 bed/grave/death/respawn 事务。下一步注入 Steamworks identity/ticket adapter，再实现 remote avatar rendering、客户端插值/reconciliation、chunk/machine source、方块编辑命令、战斗死亡 producer、墓碑 UI 和真实任务 producer/reward。
...
| 数据定义、世界生成、region 存档 | A 方案首轮已实现；`snt_voxel_data/storage` 只保留通用 terrain/chunk/raw region framing，`snt_game_world_*` 拥有游戏语义与 v12 payload；`GameWorldPersistenceLifecycle` 已在共享 simulation session 中验证/加载/关闭保存当前 dimension，region v2 与 chunk v12 只接受当前格式 |
...
| P7.3/P7.4 任务、玩家、世界与机器持久化 | 已实现：结构化 definitions、接取/推进/完成/前置解锁/repeatable、reload reconciliation、per-player `SNTQ` v1 存取、严格拒绝、backup recovery 与 dedicated-server 认证/顶号/断线/关服 lifecycle、revision-gated autosave；`SNTP` 玩家值状态的加载、断线/关服保存和 dirty-gated checkpoint；共享 session 的 current-format world load/bootstrap/shutdown-save、坏 chunk 不部分加载、v12 chunk-sidecar 的 machine restore/capture、床 anchor 与不可破坏墓碑记录；真实任务 producer/reward、方块编辑入口、战斗死亡 producer 与墓碑 UI 未实现 |
| 其余玩法迁移和持久状态 | 未完成 |
| 网络 replication | transport、wire codec、主线程 phase 编排、`SNTG` v5、local-name 认证/会话接管、graphical client connection、authoritative QuestAccept、UDP movement intent、server-owned movement/physics、snapshot/delta strict value codec、provider budget、concrete player interest/source、client remote-player value world、`SNTP` lifecycle 与 bed/grave/death/respawn server services 已实现；Steamworks ticket verifier、玩家方块编辑、chunk/machine source、战斗死亡 producer、墓碑 UI、客户端 prediction/reconciliation 与 remote avatar rendering 未实现 |
