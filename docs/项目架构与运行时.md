...
| game-owned `MachineTickSystem`、reload-safe 配方快照 | 已实现 P7.2.4：多输入槽、稳定 recipe id 选择、显式手动激活以及 `MachineInteractionService` activation context 校验已进入 game simulation；`MachineStructureValidator` 已验证 bloomery 的 3x3x2/17-cell voxel structure；炉子、pit kiln、charcoal pit、bloomery、anvil 内容已进入 AngelScript 包；认证玩家、库存、方块编辑，以及由 content registry 映射的已锚定机器放置、activation/collect 已经由主机 interaction service 接入；已提交方块编辑和 sidecar-anchor machine 已接到 P7.5 replication source，图形客户端机器交互输入绑定与生产机器 UI 仍待迁移 |
...
| 其余玩法迁移和持久状态 | 未完成 |
| 网络 replication | transport、wire codec、主线程 phase 编排、`SNTG` v11、local-name 认证/会话接管、graphical client connection、MUI LAN server list/selection、自动任务图、显式 QuestClaimReward、主机权威/客户端可信提示的 block interaction、UDP movement intent、server-owned movement/physics、snapshot/delta strict value codec、provider budget、concrete player/loaded-chunk/machine interest/source、认证账号 task-book source、client remote-player/chunk/machine/task-book value world、RLE terrain snapshot、interaction block delta、MUI 任务书、`SNTP` lifecycle 与 bed/grave/death/respawn server services 已实现；Steamworks ticket verifier、图形客户端机器交互输入绑定、生产机器 UI、战斗死亡 producer、墓碑 UI、客户端 prediction/reconciliation 与 remote avatar rendering 未实现 |
