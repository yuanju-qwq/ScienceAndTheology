...
| `snt_game_network` | 游戏 `SNTG` v10 envelope、typed login/command/unreliable movement/snapshot/delta codec、client/server 准入 handler、认证/命令/AOI/source contracts、typed account value、player v1、RLE terrain 与 machine v1 presentation codec | game-owned 且 headless；`GameAccountPeerAuthenticator` 已允许 local-name 并让同 account 新登录接管旧 peer；handler 对注入 provider 执行严格 codec + bytes/chunk/entity/value/block per-peer budget + atomic queue，`GameClientReplicationSession` 以 shared sequence 发送 reward-claim/block-interaction command 和 movement，`GameRemotePlayerWorld`、`GameClientRemoteChunkWorld`、`GameRemoteMachineWorld` 与 `GameClientQuestBookState` 只应用有序 presentation value；Steam ticket verifier、remote avatar rendering 尚未接入 |
