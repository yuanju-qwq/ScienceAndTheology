...
配置期审计会扫描注册模块的 `.h`、`.hpp`、`.inl`、`.cpp` 和 `.cxx` 文件，将带引擎模块前缀的 quoted include 映射到唯一 CMake 目标，并验证该目标直接出现在消费者的 `target_link_libraries` 中。编译目标只能通过 `PUBLIC` 或 `PRIVATE` 使用实现依赖，header-only 目标才可使用 `INTERFACE`。任何遗漏都会在生成构建文件前失败，并报告消费者目标、源码位置、include 与缺失目标。当前基线覆盖 21 个引擎目标和 136 条内部 include 边；第三方头与非构建的 `network/` 声明不在此检查范围。
...
`IAssetSource` 已由 `assets/filesystem_asset_source.*` 获得首个实现，`AssetCatalog` 已成为它的首个真实消费者。Runtime 现在从 `RuntimePaths::game_root` 创建 source，按 `RuntimeConfig::assets.manifest_path` 加载 catalog，并把两者注入 `RuntimeServices` 与 `AssetManager`；`IGpuAssetUploader` 仍只有声明：
...
- `AssetManager + VulkanMeshLoader + AssetCache` 仍是当前 GPU residency 实现，但它已只从注入的 source 读取 owned bytes，并按 catalog 顺序预分配/预加载。下一步是以 `IGpuAssetUploader` 替换该 legacy cache，而不是重新引入路径解析器。
...
- `IGpuAssetUploader`：source/catalog 已由 Runtime 注入，并由当前 AssetManager 以 source-owned bytes 驱动 legacy mesh cache；GPU residency 仍严格绑定 render thread/device 生命周期，uploader 的 token/release/evict 实现仍待完成。
...
1. **依赖基线**：已完成。`snt_engine_settings` 不再隐式提供源码根，21 个模块由配置期审计检查 136 条直接 include 依赖；CI 构建关键独立目标、测试和游戏宿主。
...
5. **数据边界**：ECS core/render/game components 已拆分；Runtime 已拥有 `FilesystemAssetSource` 和 `AssetCatalog`，并把它们注入 RuntimeServices 与 AssetManager。`IGpuAssetUploader` 仍为 mock 契约；下一步从 current AssetManager mesh cache 迁出 GPU residency，并保持 source-owned data handoff。
...
| asset catalog / GPU residency | Runtime-owned `FilesystemAssetSource` 与 source-backed `AssetCatalog` 已完成并注入 AssetManager/RuntimeServices；legacy mesh cache 已消费 source-owned bytes，GPU uploader 仍是接口声明 |
| 固定 20 TPS 单线程模拟 | 已实现 |
...
## 13. 变更记录
- 2026-07-13：Runtime 现在拥有 `FilesystemAssetSource` 和 source-backed `AssetCatalog`：由 `game_root` 与 `assets.manifest_path` 初始化，随后注入 RuntimeServices 和 AssetManager。删除旧 `AssetManager::init_from_manifest` 与直接 `load_manifest(path)` API；当前 VulkanMeshLoader 从 `AssetSourceData` 内存字节解析 OBJ，AssetManager 按 catalog 顺序预分配/预加载。`IGpuAssetUploader` 仍为下一阶段。
