...
>
> 核对范围：固定的引擎子模块基线为 `9af2c001a0d15fc6976999f6ad824f30a0c0c678`，并包含工作区中尚待提交的 `Runtime + IGameSession`、P7.2.1 gameplay 和 2026-07-12 P0 CMake 依赖隔离变更。
...
| 优先级 | 问题 | 当前影响 |
| --- | --- | --- |
| P0 | `Runtime` 已通过 `SystemScheduler` 运行生产 ECS 系统，`MachineTickSystem` 与 `PlayerPhysicsSystem` 已进入独立 worker 路径 | 已消除 `World::update` 绕过调度器的路径；机器系统在单个 task 内按稳定顺序分片，玩家碰撞只消费值快照；两个任务的资源声明无冲突，可在同一 barrier 前并发 |
| P1 | `Runtime` 已完成首轮内容剥离，但 Asset/Script/默认 Job System/路径仍为进程全局状态 | 多会话、编辑器隔离和真正无头模式仍受限 |
...
因此结论是：不需要推倒重来。新的玩法内容必须进入 `IGameSession`，不能重新写回 `Runtime`；Runtime 已使用安全调度契约，下一阶段是选择适合的纯计算系统迁移到 worker，并继续拆分全局服务。
...
flowchart TD
    ...
    Session --> Gameplay["snt_gameplay / 确定性玩法 System"]
    Gameplay --> ECS
    Gameplay --> Script
...
| `snt_gameplay` | `MachineTickSystem`、机器运行态、配方快照和事件 sink | P7.2.1 已作为 scheduler worker：主线程复制注册表/组件快照；大批机器在 worker 内分片计算，barrier 后主线程按 Guid 顺序写回 |
...
| `snt_tests` | 单一 GoogleTest 可执行程序 | 覆盖 core、asset、script、gameplay、ECS、scene、data、player、UI、RuntimeConfig；不覆盖完整 Runtime/Vulkan 启停 |
...
| `snt_gameplay` | `snt_core`、`snt_script` | `PRIVATE` | 日志和 `RegistryHub` 仅由实现使用，ECS 保持 `PUBLIC` |
...
2. CI 分别构建 `snt_core`、`snt_ecs`、`snt_render_backend`、`snt_voxel_mesh`、`snt_voxel`、`snt_gameplay` 和 `snt_engine`，再构建游戏宿主。
3. `snt_tests` 继续覆盖行为；依赖审计只验证构建图，不以运行测试替代链接隔离。
...
- 每个逻辑 tick 先调用 `IGameSession::fixed_tick`，再调用 `SystemScheduler::fixed_tick(world, 0.05f)`；`ChunkRenderSystem` 与 `PlayerControllerSystem` 在主线程处理 mesh 提交、输入和方块交互，`MachineTickSystem` 与 `PlayerPhysicsSystem` 分别捕获值快照后在 worker 执行并于 barrier 写回；机器达到阈值时，其纯计算按固定索引分片。
...
该契约已经由 metadata 校验、快照/command barrier、资源冲突 DAG、确定序命令、嵌套等待和 shutdown 测试覆盖。Runtime 已在 job system 初始化后独占 scheduler，`WorldSession` 负责系统注册，`World` 不再保存系统。当前生产注册顺序为主线程 `voxel.chunk_render`、`player.controller`，worker `gameplay.machine_tick`、`player.physics`。机器 worker 只复制 `RegistryHub` 配方和 `MachineRuntimeComponent`，不会把 World、注册表或事件 sink 带入子 job；机器数达到 32 时，父 task 将纯计算拆为 16-machine tile，完成后仍按 Guid 顺序入队。玩家 worker 在主线程从其扫掠 AABB 复制 `VoxelCollisionSnapshot`，worker 只积分 `PlayerControllerState`，再经 command queue 写回该状态和相机 Transform；它从不保留 `ChunkRegistry`、World 或组件引用。两个 worker 的资源集不相交，因此 scheduler 不建立依赖边。`ChunkRenderSystem` 保持现有跨帧异步 mesh snapshot 管线，不接入 fixed-tick barrier，以避免重 meshing 直接造成逻辑 tick 卡顿。
...
`snt_gameplay::MachineTickSystem` 已接入 Runtime 的 `SystemScheduler` worker 路径：
...
- worker 结束时仅通过 `WorldCommandQueue` 写回玩家状态和 camera Transform。它声明读取 `world.chunks`、读写 `player.controller_state`、写入 `ecs.camera_transform`，因此与机器的 `ecs.machine_runtime` / `script.registry` 路径无冲突。
...
`RuntimeServices` 由 `Runtime` 实例拥有，显式提供 `RuntimeConfig`、paths、clock、logger、jobs、assets 和 scripts；`WorldSession` 提供 World、ChunkRegistry、输入、EventBus、active camera 和鼠标锁定边界。`FrameContext` 只能在回调期间使用，`UiContext` 只允许提交 View/Arc2D draw data，不暴露 Vulkan 指针。

当前实现仍以 AssetManager、ScriptManager、默认 Job System 和路径工具的进程全局对象为底层；新 session 代码通过 `RuntimeServices` 获得显式引用，彻底删除 singleton 是第 10 节的后续工作。
...
1. **依赖基线**：已完成。`snt_engine_settings` 不再隐式提供源码根，21 个模块由配置期审计检查直接 include 依赖；CI 构建关键独立目标、测试和游戏宿主。
2. **线程基线**：已完成 `SystemMetadata`、资源冲突 DAG、fixed-tick barrier、shutdown wait、`WorldCommandQueue` 和 worker 内 `parallel_for`；Runtime 已以 scheduler 取代 `World::update`，`WorldSession` 是唯一系统注册入口。`MachineTickSystem` 提供大批机器的内部数据并行，`PlayerPhysicsSystem` 是第二个资源独立的生产 snapshot/command worker，已形成跨系统并行扇出。后续只在新的数据驱动系统能提供值快照和确定性 command 时迁移；不把 GPU 上传或 `ChunkRegistry` 直接访问放入 worker。
3. **运行时边界**：已完成首轮 `RuntimeServices`、`WorldSession`、`IGameSession` 和 `Runtime`；demo bootstrap、固定 Guid、玩家、脚本加载和玩法 UI 已移到 `game/`。`IRuntimeModule` 的调度元数据仍待设计。
4. **消除新全局状态**：Runtime 已向 session 显式提供 Asset/Script/Job 服务引用；下一步将底层 singleton 改为实例成员，并删除旧全局入口。
...
| `Runtime + IGameSession` 内容边界 | 已实现首轮；`ScienceAndTheologySession` 拥有游戏场景、脚本、玩家、试玩世界和玩法 UI |
| SDL3 窗口和输入 | 已实现 |
...
- 2026-07-12：完成 P0 CMake 依赖隔离。新增配置期模块 include 审计，`snt_engine_settings` 移除源码根 include；补齐 `input`、`ecs`、`render_backend`、`voxel`、`player` 和 `gameplay` 的直接依赖。独立关键目标与客户端 Debug 构建通过；CI 改为当前 Windows CMake 引擎链。
