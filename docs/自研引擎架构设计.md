...
配置期审计会扫描注册模块的 `.h`、`.hpp`、`.inl`、`.cpp` 和 `.cxx` 文件，将带引擎模块前缀的 quoted include 映射到唯一 CMake 目标，并验证该目标直接出现在消费者的 `target_link_libraries` 中。编译目标只能通过 `PUBLIC` 或 `PRIVATE` 使用实现依赖，header-only 目标才可使用 `INTERFACE`。任何遗漏都会在生成构建文件前失败，并报告消费者目标、源码位置、include 与缺失目标。当前基线覆盖 21 个引擎目标和 134 条内部 include 边；第三方头与非构建的 `network/` 声明不在此检查范围。
...
`IAssetSource` 已由 `assets/filesystem_asset_source.*` 获得首个实现，`AssetCatalog` 已成为它的首个真实消费者；`IGpuAssetUploader` 仍只有声明，尚未迁移 `AssetManager`：
...
- `IGpuAssetUploader::upload(GpuAssetUploadRequest)` 按值接收已拥有的源数据，返回不暴露 Vulkan 句柄的 `GpuAssetResidencyToken`；`release()` 与 `evict_unused()` 明确逻辑引用释放和延迟回收的分界。uploader 绑定 render thread/device 生命周期，device 停止后其 token 全部失效。
- `AssetManager + VulkanMeshLoader + AssetCache` 仍是唯一运行路径；下一步把 Runtime 的 game content root 与 catalog 显式注入，再替换 GPU 缓存调用链。
...
- `IAssetSource` / `IGpuAssetUploader`：`FilesystemAssetSource` 与 source-backed `AssetCatalog` 已实现并有路径、空 manifest、ID 解析和格式错误测试，但尚未注入 Runtime；GPU residency 仍严格绑定 render thread/device 生命周期，`IGpuAssetUploader` 和 `AssetManager` 迁移仍待实现。
...
1. **依赖基线**：已完成。`snt_engine_settings` 不再隐式提供源码根，21 个模块由配置期审计检查 134 条直接 include 依赖；CI 构建关键独立目标、测试和游戏宿主。
...
5. **数据边界**：ECS core/render/game components 已拆分；`FilesystemAssetSource` 与 `AssetCatalog` 已实现独立内容读取、manifest 解析和稳定 ID 查询，`IGpuAssetUploader` 仍为 mock 契约。下一步将 Runtime 的 game content root 和 catalog 注入运行时服务，再逐步从 `AssetManager` 迁出 GPU residency。
...
| asset catalog / GPU residency | `FilesystemAssetSource`、source-backed `AssetCatalog`、无 I/O manifest parser 与边界测试已完成；GPU uploader 仍是接口声明，`AssetManager` 仍直接协调当前 Vulkan mesh 缓存 |
| 固定 20 TPS 单线程模拟 | 已实现 |
...
- 2026-07-13：新增 source-backed `AssetCatalog` 与 `kAssetNotFound`。catalog 通过 `IAssetSource` 读取 manifest，拥有 manifest identity 与 immutable ID index，并把 ID 解析为相对 source request；缺失 manifest 保持非 fatal 的空 catalog 策略。`parse_manifest()` 从文件读取中抽离，当前 `load_manifest(path)` 仍供 AssetManager 使用。新增 catalog 读取、别名、缺失 manifest、未知 ID 与 parse error 测试；Runtime 和 GPU residency 尚未迁移。
- 2026-07-13：新增 `FilesystemAssetSource` 作为 `IAssetSource` 的首个实现。source 在创建时规范化并拥有内容目录根，读取时拒绝任何带 root name/root directory 的路径、根目录逃逸和非普通文件，按值返回 canonical path 与 bytes；初始化只记录一次根目录日志。新增文件源创建、规范化读取、路径逃逸和缺失文件测试。Runtime 与 `AssetManager` 尚未注入/迁移该 source，`IGpuAssetUploader` 保持声明阶段。
- 2026-07-13：新增 declaration-only `IAssetSource` 与 `IGpuAssetUploader`。前者返回拥有规范路径和字节的 source data，可用于非渲染加载线程；后者按值接收上传请求，使用 Vulkan-free residency token，并明确 release/evict 与 render-thread/device 生命周期。新增 mock 契约测试；`AssetManager`、`VulkanMeshLoader` 和现有运行路径尚未迁移。
