...
>
> 当前工作区包含 `SimulationRuntime + ISimulationSession`、`ClientRuntime + IClientSession`、`snt_game_server`、`snt_network`、`snt_game_identity`、`snt_game_network`、game-owned P7.2.1 机器运行态、P7.3 `QuestRegistry` core、P7.4 task progress provider、P0 CMake 依赖隔离和 A 方案 voxel/game world 数据边界变更。
...
| P1 | 游戏账号身份、replication envelope、登录准入状态机和 future service contracts 已实现；local-name 认证可用，Steamworks 平台 backend、命令、snapshot/delta/AOI 尚不存在，音频尚不存在 | `snt_game_identity` 以 `steam:<SteamID64>` / `local-name:<name>` 隔离账号；`snt_game_network` 已实现 `SNTG` v2、typed login/command codec、会话接管与 `GameServerReplicationHandler`，不伪造 Steam 登录或 snapshot/delta 同步 |
...
| `snt_game_simulation` | `ScienceAndTheologySimulationSession`、游戏配置、脚本内容、机器 worker、QuestRegistry、demo terrain/sidecar | client/server 共用且只链接 simulation 闭包；不含 SDL、Vulkan、scene、player 或 UI |
...
| `game/quest/quest_registry.*` + `quest_progress.h` | per-player task state、objective progress、reload reconciliation 和稳定存档值接口 | game-owned、simulation-main-thread-only；定义/进度/效果/持久化边界分离；真实 player/machine producer 和 reward sink 尚未接入 |
...
| `snt_game_network` | 游戏 `SNTG` v2 envelope、typed login/command codec、准入 handler、认证/命令/AOI/snapshot contracts | game-owned 且 headless；`GameAccountPeerAuthenticator` 已允许 local-name 并让同 account 新登录接管旧 peer；Steam ticket verifier、命令 sink、AOI 和 snapshot source 的 concrete gameplay provider 尚未接入 |
...
| `snt_game_server` | 运行包定位、配置读取、`ScienceAndTheologyServerSession` 与 TCP+UDP/`snt_game_network` 组合 | 支持 `--ticks N`、`--network`、`--bind`、`--tcp-port`、`--udp-port`；不链接 SDL/Vulkan/client runtime；local-name 准入可用，Steam ticket 无 verifier 时明确拒绝，版本错误、非可靠或未实现 gameplay frame 同样明确拒绝 |
...
`ScienceAndTheologySimulationSession::create_world` 只加载脚本、机器 worker 与试玩 terrain/sidecar；`ScienceAndTheologyClientSession::create_client_world` 才加载 scene、固定 camera Guid、玩家控制和玩法 UI，并将已生成 chunk 标记为待 mesh。脚本 watcher 轮询已移到 simulation fixed tick，因此图形客户端和无头服务端遵循同一 reload 生命周期；客户端仅保留 F5 的显式 reload 触发。`ScienceAndTheologyServerSession` 组合同一 simulation session：可选 TCP+UDP transport 只在该 wrapper 中创建，`--ticks N` 默认不监听端口，`--network`（及端口覆盖）才打开监听。关闭时 ClientRuntime 先调用 session shutdown，再停止 SimulationRuntime scheduler/job，随后销毁 render/chunk/UI/GPU assets，最后释放 simulation 的 script、world、catalog/source 和文件日志。这样 worker 不会在 world 或 GPU 资源销毁后继续访问快照结果，AssetManager 也在其借用的 source/catalog 之前释放。CMake 目标依赖已在配置期审计；运行时初始化/关闭顺序仍由手写生命周期维持，尚无通用运行时依赖图自动校验。
...
`QuestRegistry` 在相同 main-thread fixed tick 以内容 revision 同步定义，按 stable player/quest key 保存进度。`quest_progress.h` 将值记录和 `IQuestProgressPersistence` 从 runtime registry 分离，`GameSaveQuestProgressPersistence` 通过 `GameSaveManager` 在显式 universe root 下保存严格的 `SNTQ` v1 per-player 文件；`load_player_progress()` / `save_player_progress()` 只能由玩家生命周期调用，fixed tick 不读写文件。玩家/机器事件 producer、reward sink、save slot 选择和机器运行态持久化尚未实现，不能把该模块当成完整任务玩法。详细迁移顺序见 [p7_玩法迁移设计.md](p7_玩法迁移设计.md) 和 [任务系统设计.md](任务系统设计.md)。
...
游戏 server 已按“收包 -> 游戏 fixed tick -> scheduler barrier -> 发包”接入 `ReplicationService`。`snt_game_network` 的 `GameServerReplicationHandler` 现在解析游戏拥有的 `SNTG` v2 envelope：可靠 `LoginRequest` 经可注入 `IGamePeerAuthenticator` 认证后，在 barrier 后发送 `LoginAccepted`；认证后的 `Command` 只交给可注入 `IGameReplicationCommandSink`。默认 `GameAccountPeerAuthenticator` 将 local-name 映射为 `local-name:<validated name>`，同 id 的后登录在 outbound 阶段通过 `IReplicationFrameSink::disconnect()` 接管并断开旧 peer。Steam 请求必须经 `ISteamSessionTicketVerifier` 返回 SteamID64；当前 server 未注入该 adapter 时明确拒绝，绝不信任客户端声明的 SteamID。`InterestUpdate`、snapshot acknowledgement、snapshot/delta producer、Steamworks backend 和具体 gameplay command 仍仅声明或明确拒绝，不能把该基线误当为完整联机玩法。详情见 [游戏网络协议设计.md](游戏网络协议设计.md)。
...
7. **无头与网络**：已完成 transport 基线、game account identity 与首轮 game protocol/admission 边界。`snt_game_server` 通过 `ScienceAndTheologyServerSession` 可选组合 TCP+UDP 与 `snt_game_network`，以 `--ticks N` 运行完整游戏包；`snt_game_identity` 提供 Steam/local-name 名字空间、本地 `SNTI` profile 与首启 prompt contract，`snt_game_tests` 覆盖同名隔离、资料恢复、`SNTG` v2 envelope、本地同名会话接管、登录确认和未认证命令拒绝。下一步注入 Steamworks identity/ticket adapter，再实现 client connection、game command、interest、snapshot/delta。
...
| P7.3/P7.4 任务进度 | 已实现：结构化 definitions、接取/推进/完成/前置解锁/repeatable、reload reconciliation、per-player `SNTQ` v1 存取、严格拒绝和 backup recovery；真实 producer、reward effect、玩家生命周期和机器运行态持久化未实现 |
| 其余玩法迁移和持久状态 | 未完成 |
| 网络 replication | transport、wire codec、主线程 phase 编排、`SNTG` v2、local-name 认证与会话接管已实现；Steamworks ticket verifier、client connection、game command、interest、snapshot/delta 未实现 |
...
## 13. 变更记录
- 2026-07-14：完成游戏账号身份基线。新增无 SDL/Steamworks 依赖的 `snt_game_identity`：Steam 使用 `steam:<SteamID64>`，本地启动名字使用 `local-name:<validated UTF-8 name>`，所以相同显示名属于不同名字空间；本地资料保存为 `user_root/profile/local_player_identity.bin` 的严格 `SNTI` v1，首启通过 SDL 输入窗口创建，损坏拒绝且仅在 primary 缺失时使用 `.bak`。`SNTG` 升至 v2，客户端不再传递 SteamID；server 的 `GameAccountPeerAuthenticator` 只接受 `ISteamSessionTicketVerifier` 返回的 SteamID64。相同 local-name 的新登录会在 outbound 阶段接管并断开旧 peer，任务进度可直接以 account id 为稳定 key。当前没有 Steamworks adapter，因此 Steam 登录仍明确不可用，未伪造成已完成。

- 2026-07-14：完成 P7.4 的任务进度存档 provider。`QuestProgressRecord` 与 `IQuestProgressPersistence` 移入不依赖 simulation 的 `quest_progress.h`，使 `snt_game_world_save` 只依赖稳定值类型；`GameSaveManager` 新增 universe 根下 `players/player_{hex_player_id}.quest` 的 `SNTQ` v1 格式，严格拒绝非当前版本、损坏和尾随数据，写入保留 `.bak` 恢复路径。`QuestRegistry` 新增显式 load/save 编排且 fixed tick 不做 I/O。新增跨重启、尾随数据拒绝和 primary 缺失 backup recovery 测试；真实 player lifecycle、存档槽选择、任务 producer/reward 和机器运行态持久化仍待实现。

- 2026-07-14：完成 P7.3 任务运行态 core。新增 game-owned `QuestRegistry`，按 stable player id + quest id 保存 `QuestProgressRecord`，实现接取、event/inventory/tick objective、前置解锁、repeatable reset、热重载 reconciliation 与 P7.4 snapshot/restore 值边界。`GameContentRegistry` 的 QuestDefinition 改为结构化 objective，并新增 AngelScript objective/prerequisite 注册；任务状态转换仅写低频日志。`IQuestLifecycleSink`、`IQuestRewardSink` 和 `IQuestProgressPersistence` 先声明边界，真实 player/machine producer、reward provider 与文件存档未伪实现。新增 6 个 QuestRegistry/Script API CTest，Debug `snt_game_tests` 和 `snt_game_server --ticks 3` 通过。

- 2026-07-14：完成 game-owned replication protocol/admission 首轮。新增 headless `snt_game_network`：`SNTG` 12-byte versioned envelope、typed login/command codec、`GameServerReplicationHandler` 和初始 `ClosedGamePeerAuthenticator`；`IGamePeerAuthenticator`、`IGameReplicationCommandSink`、AOI 与 snapshot source 均先声明游戏所有权和线程边界。`ScienceAndTheologyServerSession` 删除嵌套 opaque handler，改为组合新模块；该初始提交不开放玩家准入。新增 4 个 game protocol/handler tests，Debug `snt_game_tests`、`snt_game_server` 和 `snt_game_server --ticks 3` 通过。

- 2026-07-14：完成网络 transport 基线。新增 `snt_network`，实现 transport-neutral `ReplicationService`、20-byte wire header、non-blocking TCP+UDP 关联握手和 Steam P2P backend adapter；TCP hello 与 UDP association 使用 128-bit token，可靠与不可靠通道有独立 payload/queue 限制。`ISimulationSession` 增加 fallible pre/post fixed tick，SimulationRuntime 固定执行收包、游戏、scheduler barrier、发包顺序。新增 `ScienceAndTheologyServerSession`，只在显式 `--network` 或 server 配置启用时监听；未知 gameplay payload 明确拒绝。独立 network tests 覆盖双向 localhost、wire 拒绝、phase 顺序和 Steam fake backend；Debug `snt_network_tests`、`snt_simulation_runtime_tests`、`snt_game_tests` server filter 和 `snt_game_server --ticks` smoke 通过。

- 2026-07-14：完成 dedicated-server 宿主基线。新增 `snt_game_simulation`，由 `ScienceAndTheologySimulationSession` 统一拥有脚本内容、机器 worker、terrain 和 game sidecar；`ScienceAndTheologyClientSession` 只保留 scene、player、输入和 UI，并以组合方式转发 simulation 生命周期。新增 `snt_game_server` 与共用 `runtime_package`，服务端支持 `--ticks N` 的完整运行包自检且不链接 SDL/Vulkan/client runtime。脚本 watcher 轮询移入 fixed tick，保证 client/server 使用同一 reload 生命周期；新增 game-level 无头启动、terrain 和 tick 集成测试。CMake 依赖审计通过，Debug server/client/game tests 构建通过，`snt_game_server --ticks 3` 与新增 CTest 通过；真实 transport、协议 codec 和 network tick 编排仍待选择和实现。
