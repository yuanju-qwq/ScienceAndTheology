...
> 当前架构基线，更新于 2026-07-11。
>
> 核对范围：固定的引擎子模块基线为 `9af2c001a0d15fc6976999f6ad824f30a0c0c678`，并包含 2026-07-11 工作区中尚待提交的 `Runtime + IGameSession` 迁移和 P7.2.1 gameplay 变更。
...
因此结论是：不需要推倒重来。新的玩法内容必须进入 `IGameSession`，不能重新写回 `Runtime`；并行调度和全局服务拆分仍是下一阶段的前置工作。
...
本阶段的目标是让“源码直接使用的模块”与“目标直接声明的模块”可机械核对；不是把源码树拆成多个物理 include 根。当前所有模块都使用 `#include "core/..."` 形式，因此仅收窄 include path 不能证明链接关系正确，必须同时建立依赖检查。

1. `snt_engine_settings` 只保留 C++ 标准、编译选项和全局编译定义，不再作为所有模块源码根目录的隐式 include 提供者。
2. 每个目标自行声明编译自身和公开头文件所需的 include 目录；公开头出现的模块类型必须通过对应目标的 `PUBLIC` 链接传播，实现文件独占的依赖必须为 `PRIVATE`。
3. 增加一个配置期检查脚本：收集每个目标源码中的 `#include "<module>/..."`，将模块目录映射到 CMake 目标，并验证该目标在 `target_link_libraries` 中直接出现。允许项必须显式列白名单并附带原因，禁止依赖最终 `snt_engine` 聚合链接“碰巧”补齐符号。
4. `snt_engine` 继续是 facade，但只能表达宿主所需的聚合集合；它不参与依赖检查的豁免，也不向下游模块反向提供头文件或链接依赖。

第一批需要修正的直接依赖如下。它们是当前公开头和实现的实际使用关系，不代表未来最终模块边界：

| 目标 | 应补充的直接依赖 | 可见性 | 原因 |
| --- | --- | --- | --- |
| `snt_input` | `snt_core` | `PUBLIC` | `input_system.h` 暴露 `core/events.h` 类型 |
| `snt_ecs` | `snt_core` | `PUBLIC` | World、事件和组件公共头使用 core 类型 |
| `snt_ecs` | `snt_assets` | `PUBLIC`，临时 | 当前 `MeshRef`/`AssetHandle` 位于 ECS 公共组件中；第 6 节的组件拆分完成后删除此依赖 |
| `snt_render_backend` | `snt_core` | `PUBLIC` | 公共 Vulkan 封装返回 `Expected` 等 core 类型 |
| `snt_render_backend` | `snt_platform` | `PRIVATE` | surface 创建实现在后端中使用 `Window`，但公开接口只需前置声明 |
| `snt_voxel` | `snt_core` | `PUBLIC` | chunk renderer / render system 公开头直接使用 core 错误与作业类型，不能只经 `snt_assets` 传递 |
| `snt_gameplay` | `snt_core`、`snt_script` | `PRIVATE` | 当前只在实现中使用低频状态日志和 `RegistryHub`；公开头仅前置声明脚本类型，ECS 依赖保持 `PUBLIC` |

验证分三层执行：

1. 配置期运行依赖检查，任何未声明 include 使配置失败。
2. CI 分别构建 `snt_core`、`snt_ecs`、`snt_render_backend`、`snt_voxel_mesh`、`snt_voxel`、`snt_gameplay` 和 `snt_engine`，而非只构建最终游戏可执行程序。
3. `snt_tests` 继续覆盖行为；新增的依赖检查只验证构建图，不以运行测试替代链接隔离。
...
`ScienceAndTheologySession` 目前在两个回调中拥有脚本加载、场景、固定 camera Guid、试玩 terrain、玩家控制器和玩法 UI。`shutdown` 先调用 session，再停止 Job System，随后释放 world/render/GPU 资源；这保证 worker 不会在 world 或 GPU 资源销毁后继续访问快照结果。GPU 资产仍在 Vulkan device 前释放。依赖顺序由 `Runtime::Impl` 和手写代码维持，尚无模块依赖图自动校验。
...
- 2026-07-11：补充 P0 依赖隔离落地契约，定义目标级公开/私有依赖规则、第一批修正项和三层验证方式；尚未实现该契约的 CMake 检查器。
