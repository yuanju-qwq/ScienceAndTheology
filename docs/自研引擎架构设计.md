...
>
> 当前工作区包含 `SimulationRuntime + ISimulationSession`、`ClientRuntime + IClientSession`、`snt_game_server`、`snt_network`、`snt_game_network`、game-owned P7.2.1 机器运行态、P0 CMake 依赖隔离和 A 方案 voxel/game world 数据边界变更。
...
| P1 | 游戏 replication envelope、登录准入状态机和 future service contracts 已实现；具体认证 provider、命令、snapshot/delta/AOI 与 Steamworks 平台 backend 尚不存在，音频尚不存在 | `snt_game_network` 已实现 `SNTG` versioned envelope、typed login/command codec 与 `GameServerReplicationHandler`；server 默认关闭玩家准入，不伪造 snapshot/delta 同步 |
...
| `snt_game_simulation` | `ScienceAndTheologySimulationSession`、游戏配置、脚本内容、机器 worker、demo terrain/sidecar | client/server 共用且只链接 simulation 闭包；不含 SDL、Vulkan、scene、player 或 UI |
| `game/client/game_content_registry.*` | 配方、机器、任务、事件、临时脚本状态和 `snt_*` binding | 由 `ScienceAndTheologySimulationSession` 拥有；保留内建回退、稳定 ScriptId、值拷贝与 reload 提交/回滚 |
...
| `snt_game_network` | 游戏 `SNTG` envelope、typed login/command codec、准入 handler、认证/命令/AOI/snapshot contracts | game-owned 且 headless；默认 `ClosedGamePeerAuthenticator` 拒绝登录；`IGamePeerAuthenticator`、命令 sink、AOI 和 snapshot source 的 concrete gameplay provider 尚未接入 |
...
| `snt_game_server` | 运行包定位、配置读取、`ScienceAndTheologyServerSession` 与 TCP+UDP/`snt_game_network` 组合 | 支持 `--ticks N`、`--network`、`--bind`、`--tcp-port`、`--udp-port`；不链接 SDL/Vulkan/client runtime；版本错误、非可靠或未实现 gameplay frame 明确拒绝，默认认证器拒绝玩家准入 |
...
| `snt_game_tests` | game-owned GoogleTest 可执行程序 | 覆盖玩法 UI、内容注册、热重载、机器 worker、game simulation session 无头启动/tick、game replication envelope/准入，以及 game worldgen、sidecar、serializer 和 save split |
...
游戏 server 已按“收包 -> 游戏 fixed tick -> scheduler barrier -> 发包”接入 `ReplicationService`。`snt_game_network` 的 `GameServerReplicationHandler` 现在解析游戏拥有的 `SNTG` envelope：可靠 `LoginRequest` 经可注入 `IGamePeerAuthenticator` 认证后，在 barrier 后发送 `LoginAccepted`；认证后的 `Command` 只交给可注入 `IGameReplicationCommandSink`。`InterestUpdate`、snapshot acknowledgement、snapshot/delta producer 和具体认证 provider 目前仅声明或明确拒绝，不能把该基线误当为完整联机玩法。默认 server 安装 `ClosedGamePeerAuthenticator`，因此不会在认证方案未选择时开放玩家准入。详情见 [游戏网络协议设计.md](游戏网络协议设计.md)。
...
7. **无头与网络**：已完成 transport 基线和首轮 game protocol/admission 边界。`snt_game_server` 通过 `ScienceAndTheologyServerSession` 可选组合 TCP+UDP 与 `snt_game_network`，以 `--ticks N` 运行完整游戏包；`snt_network_tests` 覆盖 localhost 双向通道、拒绝和 Steam adapter，`snt_game_tests` 覆盖 `SNTG` envelope、登录确认、未认证命令拒绝和 network-disabled server session。下一步选择并注入 concrete authenticator，再实现 game command、interest、snapshot/delta。
...
| 其余玩法迁移和持久状态 | 未完成 |
| 网络 replication | transport、wire codec、主线程 phase 编排、`SNTG` envelope 与登录/命令边界已实现；concrete authentication、game command、interest、snapshot/delta 未实现 |
...
| replication transport | `snt_network` 已实现 TCP reliable + UDP unreliable 和 Steam P2P backend adapter；`snt_game_network` 已实现游戏 envelope/登录准入边界；Steamworks platform backend、concrete authentication 和同步 payload 待实现 |
| 音频 | 未实现 |
...
## 13. 变更记录
- 2026-07-14：完成 game-owned replication protocol/admission 首轮。新增 headless `snt_game_network`：`SNTG` 12-byte versioned envelope、typed login/command codec、`GameServerReplicationHandler` 和 `ClosedGamePeerAuthenticator`；`IGamePeerAuthenticator`、`IGameReplicationCommandSink`、AOI 与 snapshot source 均先声明游戏所有权和线程边界。`ScienceAndTheologyServerSession` 删除嵌套 opaque handler，改为组合新模块；默认拒绝玩家准入，不假装已实现完整多人玩法。新增 4 个 game protocol/handler tests，Debug `snt_game_tests`、`snt_game_server` 和 `snt_game_server --ticks 3` 通过。
