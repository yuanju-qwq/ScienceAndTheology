...
>
> 当前工作区包含 `SimulationRuntime + ISimulationSession`、`ClientRuntime + IClientSession`、`snt_game_server`、`snt_network`、game-owned P7.2.1 机器运行态、P0 CMake 依赖隔离和 A 方案 voxel/game world 数据边界变更。
...
| P1 | 游戏 replication message schema 与 Steamworks 平台 backend 尚不存在，音频尚不存在 | `snt_network` 已实现 wire codec、TCP reliable + UDP unreliable、固定 tick 编排与 Steam P2P adapter；当前 server 明确拒绝未知 gameplay payload，不伪造 snapshot/delta 同步 |
...
| `snt_game_server` | 运行包定位、配置读取、`ScienceAndTheologyServerSession` 与 TCP+UDP 组合 | 支持 `--ticks N`、`--network`、`--bind`、`--tcp-port`、`--udp-port`；不链接 SDL/Vulkan/client runtime；未知 gameplay frame 明确拒绝 |
...
| `snt_game_tests` | game-owned GoogleTest 可执行程序 | 覆盖玩法 UI、内容注册、热重载、机器 worker、game simulation session 无头启动/tick，以及 game worldgen、sidecar、serializer 和 save split |
...
游戏 server 已按“收包 -> 游戏 fixed tick -> scheduler barrier -> 发包”接入 `ReplicationService`。当前 `ScienceAndTheologyServerSession::ReplicationHandler` 只接受连接生命周期，明确拒绝未知 application payload；下一阶段才声明 game-owned 登录、命令、snapshot、delta、interest/AOI 与认证 schema。该阶段不保留旧网络 API 或协议兼容层。
...
7. **无头与网络**：已完成 transport 基线。`snt_game_server` 通过 `ScienceAndTheologyServerSession` 可选组合 TCP+UDP，以 `--ticks N` 运行完整游戏包；`snt_network_tests` 覆盖 localhost 双向通道、拒绝和 Steam adapter，`snt_game_tests` 覆盖 network-disabled server session。下一步声明 game-owned message、认证和 interest/snapshot/delta 策略。
...
| 其余玩法迁移和持久状态 | 未完成 |
| 网络 replication | transport、wire codec、主线程 phase 编排已实现；gameplay command/snapshot/delta schema 未实现 |
...
| replication transport | `snt_network` 已实现 TCP reliable + UDP unreliable 和 Steam P2P backend adapter；Steamworks platform backend 与 gameplay payload schema 待实现 |
| 音频 | 未实现 |
